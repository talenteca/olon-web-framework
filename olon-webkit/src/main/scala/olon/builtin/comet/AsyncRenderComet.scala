...
/** `AsyncRenderComet` facilitates rendering anything that produces a `JsCmd`
  * independently from a page request. All you have to do is create one and send
  * it a `Compute` message with the function that will produce the `JsCmd`.
  * `AsyncRenderComet` will take ownership of the function and run it in a
  * separate thread comet context, sending the results down using a
  ...
  *   - The `lazy` snippet.
  *   - The `CanBind` implicits in the `olon.http` package that allow using
...
object AsyncRenderComet {
  ...
  /** If you're going to be managing the asynchronicity of the render
    * externally, make sure to call this so that the async plumbing will be set
    * up on the page when it gets sent down.
    *
  ...
  /** Render the given function on a separate thread and send the resulting
    ...
    * function so that it is executed in the current request and session
    * context.
    *
    * Returns a `Failure` if something went wrong with setting up the
    * asynchronous render.
