...
/** Enclose your snippet tags on your template with LazyLoad and the snippet
  * will execute on a different thread, which avoids blocking the page render.
  */
object LazyLoad extends DispatchSnippet {
  ...
  /** If you need to provide a custom `renderer` function, perhaps because you
    ...
    *     `S.attr("template")` is set and use that template if avilable, and
    *     then fall back on the default placeholder template.
    *   - Handle invoking the `AsyncRenderComet` correctly.
    *
    * The `renderer` function will be wrapped in current request state you must
    * do this manually using `buildDeferredFunction`. This method is for
    * advanced use; most folks will probably want to interact with the snippet
    * by just wrapping their snippet invocation in a `data-lift="lazy-load"`
    * snippet.
    */
  def render(
  ...
  /** Enclose your snippet like this:
    ...
    * }}}
    *
    * Note that this must be a single element that will be replaced when the
    * lazy content is loaded. By default, an AJAX spinner is displayed inside a
    * `div`.
    *
