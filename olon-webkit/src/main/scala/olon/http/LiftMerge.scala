  ...
  def merge(xhtml: NodeSeq, req: Req): Node = {
    ...
    def waitUntilSnippetsDone(): Unit = {
      ...
      snippetHashs.synchronized {
        if (
          myMillis >= waitUntil || snippetHashs.isEmpty || !snippetHashs.values.iterator
            .contains(Empty)
        ) ()
        else {
    ...
    var htmlElement =
      <html xmlns="http://www.w3.org/1999/xhtml" xmlns:lift='http://olon.net'/>
      ...
      nodes.foldLeft(NodesAndEventJs(Vector[Node](), Noop)) {
        case (soFar, node) =>
          val childInfo =
            node match {
              case element: Elem
                  if element.label == "html" && !htmlDescendant =>
                htmlElement = element
