...
object UserAgentCalculator extends Factory {
  ...
  /** You can change the mechanism by which the user agent for IE is calculated
    * by doing the Factory thing with this object
    */
  // SCALA3 Using `?` instead of `_`
  object ieCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](defaultIeCalcFunction)
  ...
  /** You can change the mechanism by which the user agent for Safari is
    * calculated by doing the Factory thing with this object
    */
  object safariCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](
        // SCALA3 Using `?` instead of `_`
        defaultSafariCalcFunction
      )
  ...
  /** You can change the mechanism by which the user agent for Firefox is
    * calculated by doing the Factory thing with this object
    */
  object firefoxCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](
        // SCALA3 Using `?` instead of `_`
        defaultFirefoxCalcFunction
      )
  ...
  /** You can change the mechanism by which the user agent for Chrome is
    * calculated by doing the Factory thing with this object
    */
  // SCALA3 Removing `_` for passing function as a value
  object chromeCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](
        defaultChromeCalcFunction
      )

  /** You can change the mechanism by which Lift calculates if the User-Agent
    * represents an iPhone. Put your special calculation function in here
    */
  object iPhoneCalcFunction
      extends FactoryMaker[Box[Box[String] => Boolean]](Empty)

  /** You can change the mechanism by which Lift calculates if the User-Agent
    * represents an iPad. Put your special calculation function in here
    */
  object iPadCalcFunction
      extends FactoryMaker[Box[Box[String] => Boolean]](Empty)
...
trait UserAgentCalculator {
  ...
  lazy val isIPhone: Boolean =
    UserAgentCalculator.iPhoneCalcFunction.vend
      ...
      .openOr(isSafari) && (userAgent
      .map(s => s.indexOf("(iPhone") >= 0)
      .openOr(false))
...
/** A FileParamHolder contains a file uploaded via a multipart form.
  ...
  */
abstract class FileParamHolder(
    val name: String,
    val mimeType: String,
    val fileName: String
) extends ParamHolder
...
/** A ThreadGlobal containing the low-level HTTPRequest and HTTPResponse
  * instances Set very, very early.
  */
...
object Req {
  ...
  def apply(
    ...
    val paramCalcInfo: AvoidGAL = new AvoidGAL(() => {
      ...
      if (
        ...
        contentType.dmap(false)(
          _.toLowerCase.startsWith("application/x-www-form-urlencoded")
        )
      ) {
        val params = localParams ++ (request.params
          .sortWith { (s1, s2) => s1.name < s2.name })
  ...
  // SCALA3 Removing `_` for passing function as a value
  var normalizeHref = _fixHref

  private def _fixHref(
      contextPath: String,
      v: Seq[Node],
      fixURL: Boolean,
      rewrite: Box[String => String]
  ): Text = {
    val hv = v.text
    val updated =
      if (
  ...
  /** Corrects the HTML content,such as applying context path to URI's, session
    * information if cookies are disabled etc.
    */
  ...
  private[olon] def defaultCreateNotFound(in: Req) =
    XhtmlResponse(
      (<html> <body>The Requested URL {
        in.contextPath + in.uri
...
/** Holds either the body or the request input stream, depending on which was
  * requested first
  */
...
/** Holds information about the content type and subtype including the q
  * parameter and extension information.
  */
final case class ContentType(
  ...
  /** Compares this to another ContentType instance based on the q and if the q
    * matches, compare based on specialization (* vs. explicit and then order.
    */
...
/** The ContentType companion object that has helper methods for parsing Accept
  * headers and other things that contain multiple ContentType information.
  */
object ContentType {

  /** Parse the String into a series of ContentType instances, returning the
    * multiple ContentType instances
    */
...
class Req(
  ...
  /** Build a new Req, the same except with a different path. Useful for
    * creating Reqs with sub-paths.
    */
  def withNewPath(newPath: ParsePath): Req = {
    ...
    new Req(
      ...
      // SCALA3 Using `?` instead of `_`
      override lazy val location: Box[Loc[?]] = outer.location
  ...
  /** Returns true if the X-Requested-With header is set to XMLHttpRequest.
    *
    * Most ajax frameworks, including jQuery and Prototype, set this header when
    * doing any ajax request.
    */
  def ajax_? =
  ...
  /** The POST or PUT body. This will be empty if the content type is
    * application/x-www-form-urlencoded or a multipart mime. It will also be
    * empty if rawInputStream is accessed
    */
  def body: Box[Array[Byte]] = _body.flatMap(_.body)

  /** The raw input stream of a POST or PUT that is not
    * application/x-www-form-urlencoded or a multipart mime and if this method
    * is called before the body method. Remember to close the stream when done.
    */
  ...
  /** The JValue representation of this Req's body, if the body is JSON-parsable
    * AND the content-type of the request is JSON. Returns a Failure if the
    * request is not considered a JSON request (see json_?), or if there was an
  ...
  /** The Elem representation of this Req's body, if the body is XML-parsable
    * AND the content-type of the request is XML. Returns a Failure if the
    * request is not considered a XML request (see xml_?), or if there was an
    * error parsing the XML.
    *
    * If you want to forcibly evaluate the request body as XML, ignoring content
    * type, see `forcedBodyAsXml`.
    */
  ...
  /** The SiteMap Loc associated with this Req
    */
  // SCALA3 Using `?` instead of `_`
  lazy val location: Box[Loc[?]] = LiftRules.siteMap.flatMap(_.findLoc(this))

  /** Test the current SiteMap Loc for access control to insure that this Req is
...
/** This case class is used for pattern matching. See LiftRules.statelessRewrite
  * and LiftRules.statefulRewrite
  */
...
/** Maintains the context of resolving the URL when cookies are disabled from
  * container. It maintains low coupling such as code within request processing
  * is not aware of the actual response that encodes the URL.
  */
