...
/** An object representing the current state of the HTTP request and response.
  ...
  * local session info without passing a huge state construct around. The S
  * object is initialized by LiftSession on request startup.
  *
  * @see
  *   LiftSession
  * @see
...
object S extends S {
  ...
  /** RewriteHolder holds a partial function that re-writes an incoming request.
    * It is used for per-session rewrites, as opposed to global rewrites, which
    * are handled by the LiftRules.rewrite RulesSeq. This case class exists so
    * that RewritePFs may be manipulated by name. See S.addSessionRewriter for
    * example usage.
    *
    * @see
    *   # sessionRewriter
  ...
  /** DispatchHolder holds a partial function that maps a Req to a LiftResponse.
    * It is used for per-session dispatch, as opposed to global dispatch, which
    * are handled by the LiftRules.dispatch RulesSeq. This case class exists so
    * that DispatchPFs may be manipulated by name. See
    * S.addHighLevelSessionDispatcher for example usage.
    *
    * @see
    *   LiftResponse
    * @see
  ...
  /** The CookieHolder class holds information about cookies to be sent during
    * the session, as well as utility methods for adding and deleting cookies.
  ...
  object PFPromoter {
    implicit def fromPF[A, B](pf: PartialFunction[A, B]): PFPromoter[A, B] =
      new PFPromoter[A, B](() => pf)

    implicit def fromFunc[A, B](
        pff: () => PartialFunction[A, B]
    ): PFPromoter[A, B] = new PFPromoter[A, B](pff)
  }

  private[http] class ProxyFuncHolder(proxyTo: AFuncHolder, _owner: Box[String])
      extends AFuncHolder {
  ...
  private final class BinFuncHolder(
      val func: FileParamHolder => Any,
      val owner: Box[String]
  ) extends AFuncHolder
      with Serializable {
    def apply(in: List[String]): Unit = {
      logger.info(
  ...
  object BinFuncHolder {
    def apply(func: FileParamHolder => Any): AFuncHolder =
      new BinFuncHolder(func, Empty)

    def apply(func: FileParamHolder => Any, owner: Box[String]): AFuncHolder =
      new BinFuncHolder(func, owner)
  }

  object SFuncHolder {
    def apply(func: String => Any): AFuncHolder = new SFuncHolder(func, Empty)

    def apply(func: String => Any, owner: Box[String]): AFuncHolder =
      new SFuncHolder(func, owner)
  }

  /** Impersonates a function that is executed on HTTP requests from client. The
    * function takes a String as the only parameter and returns an Any.
    */
  ...
  object LFuncHolder {
    def apply(func: List[String] => Any): AFuncHolder =
      new LFuncHolder(func, Empty)

    def apply(func: List[String] => Any, owner: Box[String]): AFuncHolder =
      new LFuncHolder(func, owner)
  }

  /** Impersonates a function that is executed on HTTP requests from client. The
    * function takes a List[String] as the only parameter and returns an Any.
    */
  private final class LFuncHolder(
      val func: List[String] => Any,
      val owner: Box[String]
  ) extends AFuncHolder
      with Serializable {
  ...
  object NFuncHolder {
    ...
    def apply(func: () => Any, owner: Box[String]): AFuncHolder =
      new NFuncHolder(func, owner)
  }

  /** Impersonates a function that is executed on HTTP requests from client. The
    * function takes zero arguments and returns an Any.
    */
...
trait S extends HasParams with Loggable with UserAgentCalculator {
  ...
  /** This is simply a flag so that we know whether or not the state for the S
    * object has been initialized for our current scope.
    *
    * @see
    *   # inStatefulScope_ ?
    * @see
  ...
  /** The _snippetMap holds mappings from snippet names to snippet functions.
    * These mappings are valid only in the current request. This val is
    * typically modified using the mapSnippet method.
    *
    * @see
    *   # mapSnippet
    * @see
    *   # locateMappedSnippet
    */
  private[http] object _snippetMap
      extends RequestVar[Map[String, NodeSeq => NodeSeq]](Map())

  /** Holds the attributes that are set on the current snippet tag. Attributes
    * are available to snippet functions via the S.attr and S.attrs methods. The
    * attributes are held in a tuple, representing (most recent attributes, full
    * attribute stack).
    *
    * @see
    *   # attrs
  ...
  private[http] object _statefulSnip
      extends RequestVar[Map[String, DispatchSnippet]](Map())
  ...
  private object postFuncs
      extends TransientRequestVar(new ListBuffer[() => Unit])

  /** During an Ajax call made on a Comet component, make the Req's params
    * available
    */
  private object paramsForComet
      extends TransientRequestVar[Map[String, List[String]]](Map())

  /** We can now collect JavaScript to append to the outgoing request, no matter
    * the format of the outgoing request
    */
  private object _jsToAppend extends TransientRequestVar(new ListBuffer[JsCmd])

  private object _globalJsToAppend
      extends TransientRequestVar(new ListBuffer[JsCmd])
  ...
  // callback.
  private[olon] object requestCometVersions
      extends TransientRequestVar[Set[CometVersionPair]](Set.empty)

  private object p_queryLog
      extends TransientRequestVar(new ListBuffer[(String, Long)])
  private object p_notice
      extends TransientRequestVar(
        new ListBuffer[(NoticeType.Value, NodeSeq, Box[String])]
      )

  /** This method returns true if the S object has been initialized for our
    * current scope. If the S object has not been initialized then functionality
    * on S will not work.
    */
  ...
  /** Get a Req representing our current HTTP request.
    *
    * @return
    *   A Full(Req) if one has been initialized on the calling thread, Empty
    *   otherwise.
    *
    * @see
    *   Req
  ...
  /** If this is an Ajax request, return the original request that created the
    ...
    * which is helpful for localization purposes.
    *
    * @return
  ...
  /** An exception was thrown during the processing of this request. This is
    * tested to see if the transaction should be rolled back
    */
  ...
  /** @return
    *   a List of any Cookies that have been set for this Response. If you want
  ...
  def receivedCookies: List[HTTPCookie] =
    for (
      rc <- Box.legacyNullTest(_responseCookies.value).toList; c <- rc.inCookies
    )
  ...
  /** @return
    *   a List of any Cookies that have been added to the response to be sent
    *   back to the user. If you want the cookies that were sent with the
    *   request, see receivedCookies.
    *
    * @see
    *   olon.http.provider.HTTPCookie
    * @see
  ...
  def responseCookies: List[HTTPCookie] =
    Box.legacyNullTest(_responseCookies.value).toList.flatMap(_.outCookies)

  /** Adds a Cookie to the List[Cookies] that will be sent with the Response.
    *
    * If you wish to delete a Cookie as part of the Response, use the
    * deleteCookie method.
    *
    * An example of adding and removing a Cookie is:
    *
    * <pre name="code" class="scala" > import olon.http.provider.HTTPCookie
    *
    ...
    * \=> S.deleteCookie(cookieName), "Delete Cookie") ) } } </pre>
    *
    * @see
    *   olon.http.provider.HTTPCookie
    * @see
  ...
  /** Returns the Locale for this request based on the
    * LiftRules.localeCalculator method.
    *
    * @see
  ...
  /** A boolean indicating whether or not the response should be rendered with
    ...
    *
    * @return
    *   <code>true</code> if this response should be rendered in IE 6/7/8
    *   compatibility mode.
    *
    * @see
  ...
  /** Return a List of the LiftRules.DispatchPF functions that are set for this
    ...
    *   # clearHighLevelSessionDispatcher
    */
  def highLevelSessionDispatcher: List[LiftRules.DispatchPF] =
    highLevelSessionDispatchList.map(_.dispatch)
  ...
  /** Adds a dispatch function for the current session, as opposed to a global
    ...
    * user to be able to download a document only when logged in. First, we
    * define a dispatch function to handle the download, specific to a given
    * user:
    *
    * <pre name="code" class="scala" > def getDocument(userId : Long)() :
    * Box[LiftResponse] = { ... } </pre>
    *
    * Then, in the login/logout handling snippets, we could install and remove
    * the custom dispatch as appropriate:
    *
    * <pre name="code" class="scala" > def login(xhtml : NodeSeq) : NodeSeq = {
    ...
    * S.clearHighLevelSessionDispatcher } } </pre>
    *
  ...
  /** Adds a per-session rewrite function. This can be used if you only want a
    ...
    * rewrites take priority over rewrites set in LiftRules.rewrite, so you can
    * use this mechanism to override global functionality. For example, you
    * could set up a global rule to make requests for the "account profile" page
    * go back to the home page by default:
    *
    * <pre name="code" class="scala" > package bootstrap.olon ... imports ...
    * class Boot { def boot { LiftRules.rewrite.append { case
    * RewriteRequest(ParsePath(List("profile")), _, _, _) =>
    * RewriteResponse(List("index")) } } } </pre>
    *
    * Then, in your login snippet, you could set up a per-session rewrite to the
    * correct template:
    *
    * <pre name="code" class="scala" > def loginSnippet (xhtml : NodeSeq) :
    ...
    *
    * <pre name="code" class="scala" > def doLogout () {
    * S.removeSessionRewriter("profile") // or S.clearSessionRewriter } </pre>
    *
    * @param name
    *   A name for the rewrite function so that it can be replaced or deleted
    *   later.
    * @param rw
  ...
  /** Add a comet to the list of comets that should be registered to receive
    * updates on the page currently being rendered or on the page that invoked
  ...
  /** As with {findOrBuildComet[T]}, but specify the type as a `String`. If the
    ...
    *
    * If `receiveUpdates` is `true`, updates to this comet will be pushed to the
    * page currently being rendered or to the page that is currently invoking an
  ...
  /** Sometimes it's helpful to accumulate JavaScript as part of servicing a
    * request. For example, you may want to accumulate the JavaScript as part of
    * an Ajax response or a Comet Rendering or as part of a regular HTML
    * rendering. Call `S.appendJs(jsCmd)`. The accumulated Javascript will be
    * emitted as part of the response, wrapped in an `OnLoad` to ensure that it
    * executes after the entire dom is available. If for some reason you need to
    * run javascript at the top-level scope, use appendGlobalJs.
    */
  ...
  /** Add javascript to the page rendering that will execute in the global
    * scope. Usually you should use `appendJs`, so that the javascript runs
    * after the entire dom is available. If you need to declare a global var or
    * you want javascript to execute immediately with no guarantee that the
    * entire dom is available, you may use `appendGlobalJs`.
    */
  ...
  /** Test the current request to see if it's a POST. This is a thin wrapper
    ...
    *
    * @return
    *   <code>true</code> if the request is a POST request, <code>false</code>
    *   otherwise.
    */
  def post_? = request.map(_.post_?).openOr(false)

  /** Localize the incoming string based on a resource bundle for the current
    * locale. The localized string is converted to an XML element if necessary
    * via the <code>LiftRules.localizeStringToXml</code> function (the default
    * behavior is to wrap it in a Text element). If the lookup fails for a given
    * resource bundle (e.g. a null is returned), then the
    * <code>LiftRules.localizationLookupFailureNotice</code> function is called
    ...
    *
    * @return
    *   A Full box containing the localized XML or Empty if there's no way to do
    *   localization
    *
    * @see
    *   # locale
  ...
  /** Get a List of the resource bundles for the current locale. The resource
    * bundles are defined by the LiftRules.resourceNames and
    * LiftRules.resourceBundleFactories variables. If you do not define an entry
    * for a particular key, we fall back to using Lift's core entries.
    *
    * @see
    *   LiftRules.resourceNames
    * @see
  ...
  def resourceBundles: List[ResourceBundle] =
    resourceBundles(locale) ++ liftCoreResourceBundle.toList
  ...
  private object resourceValueCache
      extends TransientRequestMemoize[(String, Locale), String]

  /** Get a localized string or return the original string. We first try your
    * own bundle resources, if that fails, we try Lift's core bundle.
    *
    * @param str
  ...
  def ?(str: String): String =
    resourceValueCache.get(str -> locale, ?!(str, resourceBundles))
  ...
  def ?(str: String, locale: Locale): String =
    resourceValueCache.get(str -> locale, ?!(str, resourceBundles(locale)))

  /** Attempt to localize and then format the given string. This uses the
    * String.format method to format the localized string. We first try your own
    * bundle resources, if that fails, we try Lift's core bundle.
    *
    * @param str
    *   the string to localize
    * @param params
  ...
  /** Test the current request to see if it's a GET. This is a thin wrapper on
    * Req.get_?
    *
    * @return
    *   <code>true</code> if the request is a GET, <code>false</code> otherwise.
    *
    * @see
    *   Req.get_ ?
    */
  ...
  /** Returns the logical page_id of the current request. All RequestVars for a
    * current page share this id.
    */
  def renderVersion: String = RenderVersion.get

  /** The URI of the current request (not re-written). The URI is the portion of
    * the request URL after the context path. For example, with a context path
    * of "myApp", Lift would return the following URIs for the given requests:
    *
    * <table> <tr align=left> <th>HTTP request</th><th>URI</th> </tr> <tr>
    * <td>http://foo.com/myApp/foo/bar.html<td><td>/foo/bar.html</td> </tr> <tr>
    * <td>http://foo.com/myApp/test/<td><td>/test/</td> </tr> <tr>
    * <td>http://foo.com/myApp/item.html?id=42<td><td>/item.html</td> </tr>
    * </table>
    *
    * If you want the full URI, including the context path, you should retrieve
    * it from the underlying HTTPRequest. You could do something like:
    *
    * <pre name="code" class="scala" > val fullURI =
    * S.request.map(_.request.getRequestURI) openOr ("Undefined") </pre>
    *
    * The URI may be used to provide a link back to the same page as the current
    * request:
    *
    * <pre name="code" class="scala" > bind(..., "selflink" -> SHtml.link(S.uri,
    * { () => ... }, Text("Self link")), ...) </pre>
    *
    * @see
    *   Req.uri
  ...
  /** If true, then the xml header at the beginning of the returned XHTML page
  ...
  /** Redirects the browser to a given URL. Note that the underlying mechanism
    * for redirects is to throw a ResponseShortcutException, so if you're doing
    * the redirect within a try/catch block, you need to make sure to either
    * ignore the redirect exception or rethrow it. Two possible approaches would
    * be:
    *
    * <pre name="code" class="scala" > ... try { // your code here
    * S.redirectTo(...) } catch { case e: Exception if
    * !e.isInstanceOf[LiftFlowOfControlException] => ... } </pre>
    *
    * or
    *
    * <pre name="code" class="scala" > ... try { // your code here
    * S.redirectTo(...) } catch { case rse: LiftFlowOfControlException => throw
    * rse case e: Exception => ... } </pre>
    *
    * @param where
  ...
  def redirectTo(where: String): Nothing =
    throw ResponseShortcutException.redirect(where)

  /** Redirects the browser to a given URL and registers a function that will be
    * executed when the browser accesses the new URL. Otherwise the function is
    * exactly the same as S.redirectTo(String), which has example documentation.
    * Note that if the URL that you redirect to must be part of your web
    * application or the function won't be executed. This is because the
    * function is only registered locally.
    *
    * @param where
    *   The new URL to redirect to.
    * @param func
  ...
  /** Redirects the browser to a given URL. Note that the underlying mechanism
    ...
    * !e.instanceOf[LiftFlowOfControlException] => ... } </pre>
    *
    * or
    *
    * <pre name="code" class="scala" > ... try { // your code here
  ...
  def seeOther[T](where: String): T =
    throw ResponseShortcutException.seeOther(where)

  /** Redirects the browser to a given URL and registers a function that will be
    * executed when the browser accesses the new URL. Otherwise the function is
    * exactly the same as S.seeOther(String), which has example documentation.
    * Note that if the URL that you redirect to must be part of your web
    * application or the function won't be executed. This is because the
    * function is only registered locally.
    *
  ...
  private[http] object oldNotices
      extends TransientRequestVar[
        Seq[(NoticeType.Value, NodeSeq, Box[String])]
      ](Nil)

  /** Initialize the current request session. Generally this is handled by Lift
    * during request processing, but this method is available in case you want
    * to use S outside the scope of a request (standard HTTP or Comet).
    *
    * @param request
  ...
  /** Log a query for the given request. The query log can be tested to see if
    * queries for the particular page rendering took too long. The query log
    * starts empty for each new request. olon.mapper.DB.queryCollector is a
    * method that can be used as a log function for the
    * olon.mapper.DB.addLogFunc method to enable logging of Mapper queries. You
    * would set it up in your bootstrap like:
    *
    * <pre name="code" class="scala" > import olon.mapper.DB import olon.http.S
    * class Boot { def boot { ... DB.addLogFunc(DB.queryCollector) ... } }
    * </pre>
    *
    * Note that the query log is simply stored as a List and is not sent to any
    * output by default. To retrieve the List of query log items, use
    * S.queryLog. You can also provide your own analysis function that will
    * process the query log via S.addAnalyzer.
    *
    * @see
    *   # queryLog
    * @see
  ...
  /** Given a snippet class name, return the cached or predefined stateful
    * snippet for that class
    */
  def snippetForClass(cls: String): Box[DispatchSnippet] =
    _statefulSnip.is.get(cls)

  /** Register a stateful snippet for a given class name. Only registers if the
  ...
  /** Add a query analyzer (passed queries for analysis or logging). The
    * analyzer methods are executed with the request, total time to process the
    * request, and the List of query log entries once the current request
    * completes.
    *
    * @see
    *   # logQuery
  ...
  /** You can wrap the handling of an HTTP request with your own wrapper. The
    * wrapper can execute code before and after the request is processed (but
    * still have S scope). This allows for query analysis, etc. See
    * S.addAround(LoanWrapper) for an example. This version of the method takes
    * a list of LoanWrappers that are applied in order. This method is *NOT*
    * intended to change the generated HTTP request or to respond to requests
    * early. LoanWrappers are there to set up and take down state *ONLY*. The
    * LoanWrapper may be called outside the scope of an HTTP request (e.g., as
    * part of an Actor invocation).
    *
    * @see
  ...
  /** You can wrap the handling of an HTTP request with your own wrapper. The
    * wrapper can execute code before and after the request is processed (but
    * still have S scope). This allows for query analysis, etc. Wrappers are
    * chained, much like servlet filters, so you can layer processing on the
    * request. As an example, let's look at a wrapper that opens a resource and
    * makes it available via a RequestVar, then closes the resource when
    * finished:
    *
    * <pre name="code" class="scala" > import olon.http.{ ResourceVar,S } import
    ...
    * } ) ... } } </pre> This method is *NOT* intended to change the generated
    * HTTP request or to respond to requests early. LoanWrappers are there to
    * set up and take down state *ONLY*. The LoanWrapper may be called outside
  ...
  /** Get a list of the logged queries. These log entries are added via the
    * logQuery method, which has a more detailed explanation of usage.
    *
    * @see
  ...
  /** Sets a HTTP response header attribute. For example, you could set a "Warn"
    * header in your response:
    *
    * <pre name="code" class="scala" > ... S.setHeader("Warn", "The cheese is
    * old and moldy") ... </pre>
    *
    * @see
    *   # getHeaders
  ...
  /** Returns the currently set HTTP response headers as a List[(String,
    * String)]. To retrieve a specific response header, use S.getResponseHeader.
    * If you want to get request headers (those sent by the client), use
    * Req.getHeaders or S.getRequestHeader.
    *
    * @see
  ...
  /** Returns the current value of the given HTTP request header as a Box. This
    * is really just a thin wrapper on Req.header(String). For response headers,
    * see S.getHeaders, S.setHeader, or S.getHeader.
    *
    * @param name
    *   The name of the HTTP header to retrieve
    * @return
  ...
  /** Sets the document type for the response. If this is not set, the DocType
    * for Lift responses defaults to XHTML 1.0 Transitional.
    *
    * @see
    *   getDocType
    * @see
  ...
  def setDocType(what: Box[String]): Unit = {
    Box.legacyNullTest(_responseHeaders.value).foreach(rh => rh.docType = what)
  }

  /** Returns the document type that was set for the response. The default is
  ...
  /** When this is true, Lift will not emit a DocType definition at the start of
    * the response content. If you're sending XHTML and this is set to true, you
    * need to include the DocType in your template.
    *
    * @see
  ...
  /** Sets Lift's DocType behavior. If this is set to true, Lift will not emit a
    * DocType definition at the start of the response content. If you're sending
    * XHTML and this is set to true, you need to include the DocType in your
    * template.
    *
    * @param skip
    *   Set to <code>true</code> to prevent Lift from emitting a DocType in its
    *   response
    *
    * @see
    *   # skipDocType
  ...
  /** Adds a cleanup function that will be executed at the end of the request
    * pocessing. Exceptions thrown from these functions will be swallowed, so
    * make sure to handle any expected exceptions within your function.
    *
    * @param f
  ...
  private def doStatefulRewrite(old: Box[Req]): Box[Req] = {
    ...
    old.map { req =>
      if (
        statefulRequest_? && req.path.partPath.nonEmpty && (req.request ne null)
      ) {
        Req(
          req,
  ...
  /** @return
    *   a List[Cookie] even if the underlying request's Cookies are null.
    */
  ...
  /** This method is a convenience accessor for LiftRules.loggedInTest. You can
    * define your own function to check to see if a user is logged in there and
    * this will call it.
    *
    * @see
  ...
  /** Functions that are mapped to HTML elements are, by default, garbage
    * collected if they are not seen in the browser in the last 10 minutes
    * (defined in LiftRules.unusedFunctionsLifeTime). In some cases (e.g., JSON
    * handlers), you may want to extend the lifespan of the functions to the
    * lifespan of the session.
    *
    * @param span
    *   If <code>true</code>, extend the mapped function lifetime to the life of
    *   the session
    * @param f
  ...
  /** Returns whether functions are currently extended to the lifetime of the
    * session.
    *
    * @return
  ...
  /** Get a list of current attributes. Each attribute item is a pair of
    ...
    * prefixed or not. If the attribute is prefixed, the key is a Right((prefix,
    * name)). If the attribute is unprefixed then the key is a Left(name). For
    * example, the following table shows how various tag attributes would be
    * represented:
    *
    * <table> <tr> <th>Snippet Tag</th> <th>Parsed attrs</th> </tr> <tr>
    * <td>&lt;lift:MySnippet testname="test" /&gt;</td>
    * <td>List((Left("testname"), "test"))</td> </tr> <tr>
    * <td>&lt;lift:MySnippet anchor:name="test" /&gt;</td>
    * <td>List((Right(("anchor", "name")), "test"))</td> </tr> </table>
    *
    * <p>The prefixedAttrsToMap method provides a convenient way to retrieve
    * only attributes with a given prefix. The prefixedAttrsToMetaData method
    * can be used to add attributes onto an XML node</p>
    *
    * @see
  ...
  /** Returns the S attributes that are prefixed by 'prefix' parameter as a
    * Map[String, String] that will be 'merged' with the 'start' Map
    *
    * @param prefix
  ...
  /** <p>Returns the S attributes that are prefixed by 'prefix' parameter as a
    * MetaData. The start Map will be 'merged' with the Map resulted after
    * prefix matching and the result Map will be converted to a MetaData. The
    * MetaData can be used to add attributes back onto XML elements via Scala's
    * '%' method. For example, if we wanted to add attributes prefixed with
    * "anchor" to any &lt;a&gt; elements we create, we could do something
    * like:</p>
    *
    * <pre name="code" class="scala" > val myLink = (<a href= {...} >...</a>) %
    * S.prefixedAttrsToMetaData("anchor", Map("id" -> "myAnchor")) </pre>
    *
    * @param prefix
    *   the prefix to be matched
    * @param start
    ...
    *
    * @return
    *   MetaData representing the combination of current attributes plus the
    *   start Map of attributes
    *
    * @see
    *   # prefixedAttrsToMap ( String )
    * @see
  ...
  /** Similar with prefixedAttrsToMetaData(prefix: String, start: Map[String,
    * String]) but there is no 'start' Map
    */
  def prefixedAttrsToMetaData(prefix: String): MetaData =
    prefixedAttrsToMetaData(prefix, Map.empty)

  /** Converts a Map[String, String] into a MetaData instance. This can be used
    * to add attributes to an XML element based on a map of attribute->value
    * pairs. See prefixedAttrsToMetaData(String,Map) for an example.
    *
    * @param in
  ...
  /** Converts the S.attrs to a Map[String, String]. The key of the map depends
    * on whether the attribute is prefixed or not. Prefixed attributes have keys
    * of the form "prefix:name", while unprefixed attributes have keys of the
    * form "name". If you only want attributes for a specific prefix, use
    * prefixedAttrsToMap.
    *
    * @see
    *   # prefixedAttrsToMap ( String )
  ...
  /** Converts S.attrs attributes to a MetaData object that can be used to add
    * attributes to one or more XML elements. Similar to
    * prefixedAttrsToMetaData, except that it handles both prefixed and
    * unprefixed attributes. This version of the method will use all of the
    * currently set attributes from S.attrs. If you want to filter it, use the
    * attrsToMetaData(String => Boolean) version, which allows you to specify a
    * predicate function for filtering. For example, if you want all of the
    * current attributes to be added to a div tag, you could do:
    *
    * <pre name="code" class="scala" > val myDiv = (<div> {...} </div>) %
    * S.attrsToMetaData </pre>
    *
    * @return
  ...
  /** Similar to S.attrsToMetaData, but lets you specify a predicate function
    ...
    * "id" attribute, you could do:
    *
    * <pre name="code" class="scala" > val myDiv = (<div> {...} </div>) %
    * S.attrsToMetaData(_.equalsIgnoreCase("id")) </pre>
    *
    * @param predicate
    *   The predicate function which is executed for each attribute name. If the
    *   function returns <code>true</code>, then the attribute is included in
    *   the MetaData.
    *
    * @see
    *   # attrsToMetaData
  ...
  def attrsToMetaData(predicate: String => Boolean): MetaData = {
    attrs.foldLeft[MetaData](Null) {
      case (md, (Left(name), value)) if (predicate(name)) =>
        new UnprefixedAttribute(name, value, md)
  ...
  /** Converts S.attrs attributes to a MetaData object that can be used to add
    ...
    * unprefixed attributes. This version of the method will use all of the
    * currently set attributes from S.attrs. If you want to filter it, use the
    * currentAttrsToMetaData(String => Boolean) version, which allows you to
    * specify a predicate function for filtering. For example, if you want all
    * of the current attributes to be added to a div tag, you could do:
    *
    * <pre name="code" class="scala" > val myDiv = (<div> {...} </div>) %
    * S.attrsToMetaData </pre>
    *
  ...
  /** Find and process a template. This can be used to load a template from
    * within some other Lift processing, such as a snippet or view. If you just
    * want to retrieve the XML contents of a template, use Templates.apply.
    *
    * @param path
  ...
  /** Evaluate a template for snippets. This can be used to run a template from
    * within some other Lift processing, such as a snippet or view.
    *
    * @param template
  ...
  /** Used to get an attribute by its name. There are several means to getting
    * attributes:
    *
    * <pre name="code" class="scala"> // Get a Box for the attribute: val myAttr
    ...
    * "name") openOr "Not found" </pre>
    *
    * Note that this uses the data held in S.attrs, which means that it will
    * find attributes through the entire snippet nesting stack. For example,
    * given the snippets:
    *
    * <pre name="code" class="xml"> &lt;lift:MyStuff.snippetA foo="bar">
    * &lt;lift.MyStuff.snippetB>...&lt;/lift.MyStuff.snippetB>
    * &lt;/lift:MyStuff.snippetA> </pre>
    *
  ...
  object attr extends AttrHelper[Box] {
    ...
    /** Returns the unprefixed attribute value as an Option[NodeSeq] for easy
      * addition to the attributes
      */
    def ~(key: String): Option[NodeSeq] = apply(key).toOption.map(Text(_))

    /** Returns the prefixed attribute value as an Option[NodeSeq] for easy
      * addition to the attributes
      */
  ...
  /** Sometimes, in the course of eager evaluation, it becomes necessary to
    * clear attribtues so they do not polute the eagerly evaluated stuff. When
    * you need to clear the attributes, wrap your code block in clearAttrs and
    * have fun.
    *
    * @param f
  ...
  /** Initialize the current request session if it's not already initialized.
    ...
    * method is available in case you want to use S outside the scope of a
    * request (standard HTTP or Comet).
    *
    * @param session
    *   the LiftSession for this request
    * @param f
  ...
  /** Retrieves the attributes from the most recently executed snippet element.
    ...
    *
    * If you want a particular attribute, the S.currentAttr helper object
  ...
  /** Used to get an attribute by its name from the current snippet element.
    ...
    * S.currentAttr("prefix", "name") openOr "Not found" </pre>
    *
    * Note that this uses the data held in S.currentAttrs, which means that it
    * will only find attributes from the current snippet element. For example,
    * given the snippets:
    *
    * <pre name="code" class="xml"> &lt;lift:MyStuff.snippetA foo="bar">
    * &lt;lift.MyStuff.snippetB>...&lt;/lift.MyStuff.snippetB>
  ...
  def containerSession: Box[HTTPSession] =
    session.flatMap(_.httpSession).or(containerRequest.map(_.session))

  /** Returns the 'type' S attribute. This corresponds to the current Snippet's
    * name. For example, the snippet:
    *
    * <pre name="code" class="xml"> &lt;lift:Hello.world /&gt; </pre>
    *
  ...
  /** The hostname to which the request was sent. This is taken from the "Host"
  ...
  /** Clears the function map. potentially very destuctive... use at your own
    * risk!
    */
  ...
  /** The code block is executed while setting the current raw NodeSeq for
    * access elsewhere
    *
    * @param ns
    *   the current NodeSeq
  ...
  /** Set the ignore snippet error mode. In this mode, any snippet failures
    * (usually snippets not being invocable) will be ignored and the original
    * NodeSeq will be returned.
    *
    * This is useful if you want to do an initial pass of a page with a
    * white-list of snippets, but not run every snippet on the page.
    *
    * @param ignore
  ...
  def locateMappedSnippet(name: String): Box[NodeSeq => NodeSeq] =
    _snippetMap.is.get(name)

  /** Associates a name with a snippet function 'func'. This can be used to
    * change a snippet mapping on a per-request basis. For example, if we have a
    * page that we want to change behavior on based on query parameters, we
    * could use mapSnippet to programmatically determine which snippet function
    * to use for a given snippet in the template. Our code would look like:
    *
    * <pre name="code" class="scala" > import scala.xml.{ NodeSeq,Text } class
    ...
    * ... }
    *
    * def listing(xhtml : NodeSeq) : NodeSeq = { ... } </pre>
    *
    * Then, your template would simply look like:
    *
    * <pre name="code" class="scala" > &lt;lift:surround with="default"
    * at="content"&gt; ... &lt;p&gt;&lt;lift:SnipMap.topSnippet /&gt;&lt;/p&gt;
    * &lt;p&gt;&lt;lift:listing /&gt;&lt;/p&gt; &lt;/lift:surround&gt; </pre>
    *
    * Snippets are processed in the order that they're defined in the template,
    * so if you want to use this approach make sure that the snippet that
    * defines the mapping comes before the snippet that is being mapped. Also
    * note that these mappings are per-request, and are discarded after the
    * current request is processed.
    *
    * @param name
    *   The name of the snippet that you want to map (the part after
    *   "&lt;lift:").
    * @param func
    *   The snippet function to map to.
    */
  def mapSnippet(name: String, func: NodeSeq => NodeSeq): Unit = {
    _snippetMap.set(_snippetMap.is.updated(name, func))
  }

  /** The are times when it's helpful to define snippets for a certain call
    * stack... snippets that are local purpose. Use doWithSnippets to
    * temporarily define snippet mappings for the life of f.
    */
  ...
  /** Associates a name with a function impersonated by AFuncHolder. These are
    * basically functions that are executed when a request contains the 'name'
    * request parameter.
    */
  ...
  /** Decorates an URL with jsessionid parameter in case cookies are disabled
    * from the container. Also it appends general purpose parameters defined by
  ...
  private def notLiftOrScala(in: StackTraceElement): Boolean =
    in.getClassName match {
      case s if s.startsWith("olon")  => false
  ...
  /** Build a handler for incoming JSON commands based on the new Json Parser
    *
    * @param onError
    *   -- the JavaScript to execute client-side if the request is not processed
    *   by the server
    * @param f
    *   \- partial function against a returning a JsCmds
    *
  ...
  /** Build a handler for incoming JSON commands based on the new Json Parser.
    * You can use the helpful Extractor in olon.util.JsonCommand
    *
    * @param name
  ...
  implicit def stuff2ToUnpref(in: (Symbol, Any)): UnprefixedAttribute =
    new UnprefixedAttribute(in._1.name, Text(in._2.toString), Null)

  /** Attaches to this uri and parameter that has function f associated with.
    * When this request is submitted to server the function will be executed and
  ...
  /** Wrap an AFuncHolder with the current snippet and Loc context so that for
    * Ajax calls, the original snippets, RequestVars and Loc (location) are
    * populated
    *
    * @param f
  ...
  /** Maps a function that will be called with a parsed JValue and should return
  ...
  /** Set the paramsForComet and run the function
    */
  private[http] def doCometParams[T](
  ...
  def error(id: String, n: NodeSeq): Unit = {
    p_notice.is += ((NoticeType.Error, n, Full(id)))
  }
  ...
  def notice(id: String, n: NodeSeq): Unit = {
    p_notice.is += ((NoticeType.Notice, n, Full(id)))
  }
  ...
  def warning(id: String, n: NodeSeq): Unit = {
    p_notice += ((NoticeType.Warning, n, Full(id)))
  }
  ...
  private[http] def message(msg: String, notice: NoticeType.Value): Unit = {
    message(Text(msg), notice)
  }

  private[http] def message(msg: NodeSeq, notice: NoticeType.Value): Unit = {
    p_notice += ((notice, msg, Empty))
  }

  /** Add a whole list of notices
    */
  def appendNotices(
      list: Seq[(NoticeType.Value, NodeSeq, Box[String])]
  ): Unit = { p_notice.is ++= list }

  /** Returns the current notices
    */
  def getNotices: List[(NoticeType.Value, NodeSeq, Box[String])] =
    p_notice.toList

  /** Returns the current and "old" notices
    */
  def getAllNotices: List[(NoticeType.Value, NodeSeq, Box[String])] =
    p_notice.toList ++ oldNotices.is
  ...
  /** Returns the messages that are associated with any id. Messages associated
    * with the same id will be enlisted.
    *
    * @param f
    *   \- the function that returns the messages
    */
  ...
  /** Use this in DispatchPF for processing REST requests asynchronously. Note
    * that this must be called in a stateful context, therefore the S state must
    * be a valid one.
    *
    * @param f
    *   \- the user function that does the actual computation. This function
    *   takes one parameter which is the function that must be invoked for
    *   returning the actual response to the client. Note that f function is
    *   invoked asynchronously in the context of a different thread.
    */
  def respondAsync(f: => Box[LiftResponse]): () => Box[LiftResponse] = {
    RestContinuation.async { reply => reply(f.openOr(EmptyResponse)) }
  }

  /** If you bind functions (i.e. using SHtml helpers) inside the closure passed
    * to callOnce, after your function is invoked, it will be automatically
    * removed from functions cache so that it cannot be invoked again.
    */
  ...
  /** All functions created inside the oneShot scope will only be called once
    * and their results will be cached and served again if the same function is
    * invoked
    */
  def oneShot[T](f: => T): T =
