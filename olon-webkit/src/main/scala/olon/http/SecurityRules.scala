...
/** Rules for HTTPS usage by a Lift application.
  *
  * Currently corresponds directly to the
  * [[http://tools.ietf.org/html/rfc6797 HTTP `Strict-Transport-Security` header]].
  */
...
object ContentSourceRestriction {
  ...
  /** Indicates content from the given host path is allowed. See the
    * `Content-Security-Policy` spec's
    * [[https://www.w3.org/TR/CSP/#source-list-path-patching matching rules for `host-source`]]
    * for more about what this can look like.
    *
    * Example:
    * {{{
  ...
  /** Indicates inline content on the page is allowed to be interpreted. It is
    * highly recommended that this not be used, as it exposes your application
    * to cross-site scripting and other vulnerabilities.
    *
    * If not specified for JavaScript, JavaScript `on*` event handler
    * attributes, `<script>` elements, and `javascript:` URIs will not be
    * executed by a browser that supports content security policies.
    *
  ...
  /** Indicates `eval` and related functionality can be used. Some of Lift's
    ...
    * so not including this in your script sources will mean you won't be able
    * to use those.
    *
...
/** Specifies a `[[https://developer.mozilla.org/en-US/docs/Web/Security/CSP
  * Content-Security-Policy]] ` for this site. This will be sent to the client
  * in a `Content-Security-Policy` header when responses are returned from Lift.
  *
  * In development mode, content security policy violations are only reported if
  * the browser supports them, not enforced. In all other modes, content
  * security policy violations are enforced if the browser supports them.
  *
  ...
  * receive a content security policy that it can understand. See the
  * [[http://caniuse.com/#feat=contentsecuritypolicy caniuse page on content security policy]]
  * for more.
  *
  ...
  *   `[[ContentSourceRestriction.UnsafeInline UnsafeInline]]` and
  *   `[[ContentSourceRestriction.UnsafeEval UnsafeEval]]` source restrictions,
  *   though these are strongly discouraged.
  * @param styleSources
  *   A list of source restrictions for loading styles. Also accepts the
  *   `[[ContentSourceRestriction.UnsafeInline UnsafeInline]]` source, though it
  ...
  *   `[[LiftRules.contentSecurityPolicyViolationReport]]`. By default, reported
  *   to `[[ContentSecurityPolicy.defaultReportUri]]`.
  *
...
object ContentSecurityPolicy {
  ...
  /** Creates a restrictive content security policy that disallows images from
    ...
    *
    * Note that the default content security policy restricts all other
    * resources to the same origin, but allows images from any source; the
    * secure one only differs because it adds restrictions to the image sources.
    */
...
/** Specifies security rules for a Lift application. By default, HTTPS is not
  ...
  * Additionally, served pages can only be embedded in other frames from the
  * current domain.
  *
  * You can use `[[SecurityRules.secure]]` to enable more restrictive, but also
  * more secure, defaults.
  *
  * @param enforceInDevMode
