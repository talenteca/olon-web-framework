...
/** The SHtml object defines a suite of XHTML element generator methods to
...
trait SHtml extends Loggable {
  ...
  /** An attribute that can be applied to an element. Typically, this will be a
    * key-value pair, but there is a class of HTML5 attributes that should be
    * similated in JavaScript.
    */
  ...
  /** Any old attribute. You should not explicitly construct one of these, but
    * rather use "name" -> "value" and let the implicit conversion take care of
    * making a BasicElemAttr.
    */
  ...
  /** Invokes the Ajax request
    ...
    *   the JsExp that returns the request data
    * @param context
    *   defines the response callback functions and the response type
    *   (JavaScript or JSON)
    */
  ...
  /** Build a JavaScript function that will perform an AJAX call based on a
    * value calculated in JavaScript
    *
    * @param jsCalcValue
    *   the JavaScript that will be executed on the client to calculate the
  ...
  /** Build a JavaScript function that will perform an AJAX call based on a
    ...
    *   value to be sent to the server
    * @param jsContext
    *   the context instance that defines JavaScript to be executed on call
    *   success or failure
    * @param func
    *   the function to call when the data is sent
    *
  ...
  /** A wrapper around parseOpt for use in the jsonCall variants that will
    * notify the developer on the console when parseOpt is unsuccessful at
    * parsing the JSON.
    */
  ...
  /** Build a JavaScript function that will perform a JSON call based on a value
    * calculated in JavaScript. This method uses the Lift-JSON package rather
    * than the old, slow, not-typed JSONParser. This is the preferred way to do
    * client to server JSON calls.
    *
    * @param jsCalcValue
  ...
  /** Build a JavaScript function that will perform a JSON call based on a value
    ...
    *
    * The JSON generated by func will be returned to the client and passed as
    * argument to the javascript function specified in jsonContext.success
    *
    * @param jsCalcValue
    *   the JavaScript to calculate the value to be sent to the server
    * @param jsonContext
    ...
    *   success or failure
    * @param func
    *   the function to call when the JSON data is sent. The returned JSON is
  ...
  def jsonCall(
    ...
    jsonCall_*(
      jsCalcValue,
      jsonContext,
      S.SFuncHolder(s => parseOptOrLog(s).map(func) getOrElse JsonAST.JNothing)
    )
  ...
  /** Build a JavaScript function that will perform an AJAX call based on a
    ...
    *   -- the JavaScript to calculate the value to be sent to the server
    * @param ajaxContext
    *   -- the context defining the javascript callback functions and the
    *   response type
    * @param func
    *   -- the function to call when the data is sent
    *
  ...
  /** Create an Ajax button. When it's pressed, the function is executed
    ...
    * @param func
    *   -- the function to execute when the button is pushed. Return Noop if
    *   nothing changes on the browser.
    * @param attrs
  ...
  def ajaxButton(text: NodeSeq, func: () => JsCmd, attrs: ElemAttr*): Elem = {
    attrs.foldLeft(
      fmapFunc((func))(name =>
        <button onclick={
          makeAjaxCall(Str(name + "=true")).toJsCmd +
            "; return false;"
        }>{text}</button>
      )
    )((e, f) => f(e))
  ...
  /** Memoize the NodeSeq used in apply() and then call applyAgain() in an Ajax
    * call and you don't have to explicitly capture the template
    */
  ...
  /** Memoize the NodeSeq used in apply() and then call applyAgain() in an Ajax
    ...
    */
  def idMemoize(
      f: IdMemoizeTransform => NodeSeqFuncOrSeqNodeSeqFunc
  ): IdMemoizeTransform = {
  ...
  /** Create an Ajax button that when pressed, submits an Ajax request and
    * expects back a JSON construct which will be passed to the <i>success</i>
    * function
    *
    * @param text
    *   -- the name/text of the button
  ...
  def jsonButton(
    ...
    attrs.foldLeft(
      fmapFunc((func))(name =>
        <button onclick={
          makeAjaxCall(Str(name + "=true"), ajaxContext).toJsCmd +
            "; return false;"
        }>{text}</button>
      )
    )((e, f) => f(e))
  ...
  def ajaxButton(
    ...
    attrs.foldLeft(
      fmapFunc((SFuncHolder(func)))(name =>
        <button onclick={
          makeAjaxCall(
            JsRaw(name.encJs + "+'='+encodeURIComponent(" + jsExp.toJsCmd + ")")
          ).toJsCmd +
            "; return false;"
        }>{text}</button>
      )
  ...
  /** Create an Ajax button that when pressed, executes the function
    ...
    *   -- the name/text of the button
    * @param jsFunc
    *   -- the user function that will be executed. This function will receive
    *   as last parameter the function that will actually do the ajax call.
    *   Hence the user function can decide when to make the ajax request.
    * @param func
    *   -- the function to execute when the button is pushed. Return Noop if
    *   nothing changes on the browser.
    *
    * @return
    *   a button to put on your pagejsFunc.params ++
    *   List(AnonFunc(makeAjaxCall(Str(name+"=true"))))
    */
  def ajaxButton(
      text: NodeSeq,
      jsFunc: Call,
    ...
    attrs.foldLeft(
      fmapFunc((func))(name =>
        <button onclick={
          deferCall(Str(name + "=true"), jsFunc).toJsCmd + "; return false;"
        }>{text}</button>
      )
    )(_ % _)
  }
  ...
  /** This method generates an AJAX editable field.
    *
    * Normally, the displayContents will be shown, with an "Edit" button. If the
    * "Edit" button is clicked, the field will be replaced with the edit form,
    * along with an "OK" and "Cancel" button. If the OK button is pressed, the
    * form fields are submitted and the onSubmit function is called, and then
    * the displayContents are re-run to get a new display. If cancel is pressed
    * then the original displayContents are re-shown.
    *
    * Note that the editForm NodeSeq is wrapped inside of an
    * <code>ajaxForm</code>, so it can be comprised of normal (non-AJAX)
    * <code>SHtml</code> form elements. For example:
    *
    * <pre name="code" class="scala"> ajaxEditable(Test("Click me"),
    * SHtml.text("Edit me", s => println("Edited with " +s)), () => {
    * println("submitted"); Noop }) </pre>
    */
  def ajaxEditable(
    ...
    def displayMarkup: NodeSeq =
      displayContents ++ Text(" ") ++
        <input value={S.?("edit")} type="button" onclick={
          setAndSwap(editName, editMarkup, dispName).toJsCmd + " return false;"
        } />
      ...
      val formData: NodeSeq =
        editForm ++
          ...
          <input type="button" onclick={
            swapJsCmd(dispName, editName).toJsCmd + " return false;"
          } value={S.?("cancel")} />
  ...
  /** Create an anchor tag around a body which will do an AJAX call and invoke
    * the function
    *
    * @param func
    ...
    attrs.foldLeft(
      fmapFunc((func))(name =>
        <a href="javascript://" onclick={
          makeAjaxCall(Str(name + "=true")).toJsCmd + "; return false;"
        }>{body}</a>
      )
    )(_ % _)
  }
  ...
  /** This function does not really submit a JSON request to the server.
    * Instead, json is a function that allows you to build a more complex JsCmd
    * based on the JsExp <i>JE.JsRaw("this.value")</i>. This function is called
    * by the overloaded version of jsonText.
    *
    * @param value
    *   \- the initial value of the text field
    * @param ignoreBlur
    *   \- ignore the onblur event and only do the event if the enter key is
    *   pressed
    * @param json
    *   \- takes a JsExp which describes how to recover the value of the text
    *   field and returns a JsExp containing the thing to execute on blur/return
    *
    * @return
    *   a text field
    */
  def jsonText(
    ...
    (attrs.foldLeft(<input type="text" value={
      value match {
        case null => ""
        case s    => s
      }
  ...
  /** This function does not really submit a JSON request to the server.
    * Instead, json is a function that allows you to build a more complex JsCmd
    * based on the JsExp <i>JE.JsRaw("this.value")</i>. This function is called
    * by the overloaded version of jsonTextarea.
    *
    * @param value
    *   \- the initial value of the text area field
    * @param json
    *   \- takes a JsExp which describes how to recover the value of the text
    *   area field and returns a JsExp containing the thing to execute on blur
    *
    * @return
    *   a text area field
    */
  ...
  /** Generate an Area tag
    *
    * @param shape
    *   \- the shape of the area (RectShape, CircleShape, CirclePercentShape,
    *   PolyShape)
    * @param alt
  ...
  /** Make a set of Ajax radio buttons. When the buttons are pressed, the
    * function is called
    *
    * @param opts
  ...
  /** Create a select box based on the list with a default value and the
    * function to be executed on form submission with an additional selection
    * that transforms select into an ajaxText allowing the user to add a new
    * select option
    *
    * @param options
    *   -- a list of value and text pairs (value, text to display)
    ...
    *   -- the default value (or Empty if no default value)
    * @param func
    *   -- the function to execute when a new selection is made
    */
  def ajaxEditableSelect(
  ...
  /** Build a swappable visual element. If the shown element is clicked on, it
    * turns into the hidden element and when the hidden element blurs, it swaps
    * into the shown element.
    */
  ...
  /** If you want to update the href of an &lt;a> tag, this method returns a
    ...
    * when the link is clicked: <code> "#my_link" #> SHtml.hrefFunc(() =>
    * println("howdy")) </code>
    */
  ...
  /** Create something that's bindable to an event attribute and when the event
    * happens, the command will fire: <code> "input [onblur]" #> SHtml.onEvent(s
    * \=> Alert("Thanks: "+s)) </code>
    */
  def onEvent(func: String => JsCmd): GUIDJsExp =
    ajaxCall(JsRaw("this.value"), func)

  /** Create something that's bindable to an event attribute and guarded by a
    * confirm dialog. The fn passed in will only execute if the user clicks the
    * "OK" button in the confirm dialog. Useful for things like delete buttons
    * that you want a confirmation on.
    *
    * <code> ".deleteBtn [onclick]" #> SHtml.onEventIf("Are you sure you want to
    * do that?", s => deleteUser()) </code>
    */
  ...
  /** Specify the events (e.g., onblur, onchange, etc.) and the function to
    * execute on those events. Returns a NodeSeq => NodeSeq that will add the
    * events to all the Elements <code> ":text" #> SHtml.onEvents("onchange",
    * "onblur")(s => Alert("yikes "+s)) </code>
    */
  ...
  /** Execute the function when the form is submitted. This method returns a
    * function that can be applied to form fields (input, button, textarea,
    * select) and the function is executed when the form containing the field is
    * submitted.
    */
  def onSubmitUnit(func: () => Any): NodeSeq => NodeSeq =
    onSubmitImpl(func: AFuncHolder)

  /** Execute the String function when the form is submitted. This method
    * returns a function that can be applied to form fields (input, button,
    * textarea, select) and the function is executed when the form containing
  ...
  /** Execute the List[String] function when the form is submitted. This method
    * returns a function that can be applied to form fields (input, button,
    * textarea, select) and the function is executed when the form containing
    * the field is submitted.
    */
  ...
  /** Execute the Boolean function when the form is submitted. This method
    * returns a function that can be applied to form fields (input, button,
    * textarea, select) and the function is executed when the form containing
    * the field is submitted.
    */
  ...
  /** Generate an input field with type number. It allows for Double if your
  ...
  /** Create an HTML button with strOrNodeSeq as the body. The button will be
    ...
    *
    * @param strOrNodeSeq
    *   -- the String or NodeSeq (either will work just fine) to put into the
    *   body of the button
    * @param func
  ...
  /** Add appropriate attributes to an input type="submit" or button element to
    * make it submit an ajaxForm correctly and return a JsCmd to the client.
    * Note that the key difference between this and ajaxSubmit is that
    * ajaxSubmit returns a complete input type="submit" element, while
  ...
  /** Takes a form and wraps it so that it will be submitted via AJAX. This also
    * takes a parameter for script code that will be executed after the form has
    * been submitted.
    *
    * @param body
    *   The form body. This should not include the &lt;form&gt; tag.
  ...
  def ajaxForm(body: NodeSeq, onSubmit: JsCmd, postSubmit: JsCmd) =
    (<lift:form onsubmit={onSubmit.toJsCmd} postsubmit={postSubmit.toJsCmd}>{
      body
    }</lift:form>)

  /** Having a regular form, this method can be used to send the serialized
    * content of the form.
    *
    * @param formId
    *   \- the id of the form
  ...
  /** Submits a form denominated by a formId and execute the func function after
    * form fields functions are executed.
    */
  ...
  def submitAjaxForm(formId: String, postSubmit: Call): JsCmd =
    SHtml.makeAjaxCall(
      LiftRules.jsArtifacts.serialize(formId),
  ...
  private def optionToElem(option: SelectableOption[String]): Elem =
    option.attrs.foldLeft(<option value={option.value}>{option.label}</option>)(
      _ % _
    )

  private case class SelectableOptionWithNonce[+T](
      value: T,
      nonce: String,
      label: String,
      attrs: ElemAttr*
  ...
  /** Create a select box based on the list with a default value and the
    ...
    *   -- the default value (or Empty if no default value)
    * @param attrs
    *   -- the attributes to append to the resulting Elem, these may be
    *   name-value pairs (static attributes) or special HTML5 ElemAtts
    * @param onSubmit
    *   -- the function to execute on form submission
    * @param f
  ...
  /** Create a select box based on the list with a default value and the
    ...
    * resulting value was in the original list. For use with DHTML form
    * updating.
    *
    * @param opts
    *   -- the options. A list of value and text pairs
  ...
  /** Create a multiple select box based on the list with a default value and
    ...
    * the resulting value was in the original list. For use with DHTML form
    * updating.
    *
    * @param opts
    *   -- the options. A list of value and text pairs
  ...
  /** Create a select box based on the list with a default value and the
    ...
    *   -- select box attributes
    */
  private def ajaxUntrustedSelect_*(
      opts: Seq[SelectableOption[String]],
      deflt: Box[String],
      jsFunc: Box[Call],
      func: AFuncHolder,
    ...
    fmapFunc(contextFuncBuilder(testFunc)) {
      import olon.http.js.JsCmds.JsCrVar
      funcName =>
        (attrs.foldLeft(<select>{
          opts.flatMap { case option =>
            optionToElem(option) % selected(deflt.exists(_ == option.value))
  ...
  def multiSelect_*(
    ...
    fmapFunc(func)(funcName =>
      attrs.foldLeft(<select multiple="true" name={funcName}>{
        opts.flatMap(o => optionToElem(o) % selected(deflt.contains(o.value)))
      }</select>)(_ % _)
    )
  ...
  def textarea_*(value: String, func: AFuncHolder, attrs: ElemAttr*): Elem =
    fmapFunc(func)(funcName =>
      attrs.foldLeft(<textarea name={funcName}>{
        value match {
          case null => ""
          case s    => s
        }
  ...
  /** Generate a CSS transform that will bind to a template containing radio
    * buttons without overwriting the radio buttons in the template with
    * entirely new elements.
    *
    * This is a change in technique from the normal `radio` handlers in that it
    * doesn't involve the creation of a new element or injection of hidden
    * elements. That means that labels designed to target a partiular radio by
    * ID and CSS styles that target elements following radios using the `+` CSS
    * selector won't stop working once a form is bound by Lift.
    *
    ...
    * @param cssSelToValue
    *   mapping between CSS selectors of radio input nodes and values assigned
    *   to them
    */
  def radioCssSel[T](initialValue: Box[T], onSubmit: Box[T] => Any)(
      cssSelToValue: (String, T)*
  ): CssSel = {
  ...
  /** Defines a form element for a file upload that will call the specified
    * function when the file is uploaded if the file size is greater than zero.
    * Note that in order to use the fileUpload element you need to specify the
    * multipart attribute on your snippet tag:
    *
    * <pre name="code" class="xml"> &lt;lift:Some.snippet form="POST"
    * multipart="true"> ... &lt;/lift:Some.snippet> </pre>
    */
  ...
  /** Holds a series of choices: HTML for input controls alongside some user
    * defined value
    */
  case class ChoiceHolder[T](items: Seq[ChoiceItem[T]]) {

    /** Retrieve the ChoiceItem that has the given key, throwing
      * NoSuchElementException if there is no matching ChoiceItem
      */
    ...
    /** Generate a simple form by calling ChoiceItem.htmlize on each ChoiceItem
      * and concatenating the resulting HTML
      */
  ...
  /** Generate a ChoiceHolder of possible checkbox type inputs that calls back
    * to the given function when the form is submitted.
    *
    * @param possible
    *   complete sequence of possible values, each a separate checkbox when
    *   rendered
    * @param actual
  ...
  def checkbox[T](
    ...
    fmapFunc {
      ...
      ChoiceHolder(possible.toList.zipWithIndex.map { possibleChoice =>
        ChoiceItem(
          possibleChoice._1,
          attrs.foldLeft(<input type="checkbox" name={name} value={
...
class JsContext(
    override val success: Box[String],
...
object Html5ElemAttr {
  ...
  /** The placeholder attribute for HTML5.
    *
    * @param text
    *   \- a String or () => String that will be the placeholder property in the
    *   attribute
    */
...
/** Mix this trait into a snippet class so that you have a convenient value to
  * redirect back to (whence). When you're done with the snippet,
...
/** Memoize the CSS Selector Transform and the most recent NodeSeq sent to the
  * NodeSeq => NodeSeq so that when applyAgain() is called, the NodeSeq most
  * recently used in apply() is used.
  */
...
/** A mechanism to memoize a transformation and then re-use the most recent html
  * and ID to redraw the content or even use an Ajax call to update the content
  */
...
object NodeSeqFuncOrSeqNodeSeqFunc {
  ...
  ): NodeSeqFuncOrSeqNodeSeqFunc = SeqNodeSeqFunc(in)
}
...
/** A long time ago, Lift was going to track every function/GUID combination
  * vended to a web page with extreme granularity. This meant that for every
  * function/GUID vended, Lift would put that GUID in an attribute associated
  * with the element on the page. In order to capture the GUIDs, some methods
  * like SHtml.ajaxCall() returned a Tuple containing the GUID and the JsExp.
  * This caused confusion and ugly code. So, the GUIDJsExp came into being.
  * Basically, it's backward compatible with the Tuple (String, JsExp), but it
  * functions like a JsExp (although you don't even have to call .toJsCmd
  * because the toString method returns the expresion itself). It should make
  * the ajaxCall()._2.toJsCmd thing into ajaxCall().
  */
