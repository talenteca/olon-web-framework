...
object Templates {
  ...
  private def findInViews(
      ...
      part: List[String],
      last: String,
      what: List[LiftRules.ViewDispatchPF]
  ): Box[NodeSeq] =
    what match {
      case Nil => Empty
      case x :: xs =>
  ...
  /** Given a list of paths (e.g. List("foo", "index")), find the template. This
    * method runs checkForContentId on any found templates. To get the raw
    * template, use findRawTemplate
    * @param places
    *   \- the path to look in
    *
    * @return
    *   the template if it can be found
    */
  ...
  /** Check to see if the template is marked designer friendly and lop off the
    * stuff before the first surround
    */
  ...
  /** Given a list of paths (e.g. List("foo", "index")), find the template.
    * @param places
    *   \- the path to look in
    * @param locale
    *   \- the locale of the template to search for
    *
    * @return
    *   the template if it can be found
    */
    ...
    first(toTry) { clsName =>
      try {
        tryo(List(classOf[ClassNotFoundException]), Empty)(
          Class.forName(clsName).asInstanceOf[Class[AnyRef]]
        ).flatMap { c =>
...
/** An abstract exception that may be thrown during page rendering. The
  * exception is caught and the appropriate report of a SnippetError is
  * generated
  */
      ...
      .dropWhile { e =>
        {
          ...
          cn.startsWith("olon.http") ||
          cn.startsWith("olon.common") ||
          cn.startsWith("olon.util")
        }
      }
      .filter { e =>
        {
