...
/** Mix this trait into your REST service provider to convert between different
  ...
  * attributes that will be applied to your root element based on the return
  * from your API.
  *
  ...
  *
  * <pre name="code" class="scala"> object CalculatorApi extends XmlApiHelper {
  ...
  * LiftResponse = reduceOp(_ min _)(r) } </pre>
  *
  * With this API, the URL <pre>http://foo.com/api/sum?args=1,2,3,4,5</pre>
  * would return <pre name="code" class="xml"> &lt;api operation="sum"
  * success="true">&lt;result>15&lt;/result>&lt;/api> </pre>
  */
trait XMLApiHelper {

  /** Converts a boolean into a response of a root element with no contents and
    * the "success" attribute set to the value of the "in" parameter.
    */
  implicit def boolToResponse(in: Boolean): LiftResponse =
    buildResponse(in, Empty, <xml:group/>)

  /** Converts a boxed boolean into a response of a root element with no
    * contents and the "success" attribute set to the value of the "in"
    * parameter. If the Box is a Failure, the "msg" attribute of the root
    * element will be set to the Failure's msg value.
    */
  ...
  /** Converts a boxed Seq[Node] into a response. If the Box is a Full, the root
    * element uses the contents of the Box as its contents, and sets the
    * "success" attribute to "true". If the Box is a Failure, the "success"
    * attribute is set to "false" and the "msg" attribute is set to the
    * Failure's msg value. If the Box is Empty then the root element is returned
    * with no contents and the "success" attribute set to "false".
    */
  ...
  /** Converts a Seq[Node] into a root element with the "success" attribute set
    * to "true" and the Seq[Node] as the contents.
    */
  implicit def listElemToResponse(in: Seq[Node]): LiftResponse =
    buildResponse(true, Empty, in)

  /** Converts a pair of (Boolean,String) into a response of a root element with
    * no contents, the "success" attribute set to the value of the first element
    * of the pair, and the "msg" attribute set to the value of the second
    * element of the pair.
    */
  ...
  /** Determines the value to place in the "operation" attribute of the root
    * element based on the second element of the request path.
    */
