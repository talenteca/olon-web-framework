...
object JqWiringSupport {
  ...
  /** Takes two sequences, the id of a containing component and a couple of
    ...
    * @param id
    *   -- the id of the enclosing DOM node. Used for appending and inserting
    *   DOM nodes
    * @param calcId
    ...
    *
    * @return
    *   the JsCmd that inserts, appends, removes, etc. the DOM so that the DOM
    *   represents the new List
    */
...
/** Contains Scala JsExps for jQuery behaviors.
  ...
  *
  * Documentation on the case classes themselves will point to the relevant
  * jQuery documentation, if there is any.
  */
object JqJE {
  ...
  /** Calls the main jQuery (or $) function with "document". This returns the
    * jQueryied document object (e.g., for calling ready()).
    *
    * See http://api.jquery.com/jQuery/ .
    */
  ...
  /** For every passed tuple, executes the given JsCmd when the given Char is
    * pressed by the user. Watches using the jQuery keypress function.
    *
  ...
  /** Calls the jQuery empty function followed by calling the jQuery after
    * function with the given content.
    *
    * The intent is to empty the matched nodes and stick the given content at
    * their tails. Like a cleaner innerHTML.
    *
...
object JqJsCmds {
  ...
  /** Append a NodeSeq to a node specified by uid using jQuery's append()
    * method.
    */
  object AppendHtml {
  ...
  /** AppendTo a NodeSeq to a node specified by uid using jQuery's appendTo()
    * method.
    */
  object AppendToHtml {
    def apply(uid: String, content: NodeSeq): JsCmd =
  ...
  /** Prepends a NodeSeq to a node specified by uid using jQuery's prepend()
    * method.
    */
  object PrependHtml {
    def apply(uid: String, content: NodeSeq): JsCmd =
  ...
  /** Prepends a NodeSeq to a node specified by uid using jQuery prependTo()
    * method.
    */
  ...
  /** Show an element identified by uid. There are two apply methods, one takes
    * just the id, the other takes the id and timespan that represents how long
  ...
  /** Hide an element identified by uid. There are two apply methods, one takes
    * just the id, the other takes the id and timespan that represents how long
    * the animation will last
    */
  object Hide {
  ...
  /** Show a message msg in the element with id where for duration milliseconds
    * and fade out in fadeout milliseconds
    */
  ...
  object FadeOut {
    ...
    /** Fade Out with the default duration and fadeTime provided by JsRules
      */
    def apply(id: String) =
      new FadeOut(id, JsRules.prefadeDuration, JsRules.fadeTime)
  }

  /** Fades out the element having the provided id, by waiting for the given
    * duration and fading out during fadeTime
    */
  ...
  object FadeIn {
    ...
    /** Fade In with the default duration and fadeTime provided by JsRules
      */
    def apply(id: String) =
      new FadeIn(id, JsRules.prefadeDuration, JsRules.fadeTime)
  }

  /** Fades in the element having the provided id, by waiting for the given
    * duration and fading in during fadeTime and use @fadeTime
    */
