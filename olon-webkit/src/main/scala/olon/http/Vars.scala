...
/** A typesafe container for data with a lifetime nominally equivalent to the
  ...
  *
  * <code> object MySnippetCompanion { object mySessionVar extends
  * SessionVar[String]("hello") } </code>
  *
  * The standard pattern is to create a singleton object extending SessionVar
  * instead of creating an instance variable of a concrete SessionVar subclass.
  * This is preferred because SessionVar will use the name of its instantiating
  * class for part of its state maintenance mechanism.
  *
  * If you find it necessary to create a SessionVar subclass of which there may
  * be more than one instance, it is necessary to override the __nameSalt()
  * method to return a unique salt value for each instance to prevent name
  * collisions.
  *
  * Note: SessionVars can be used within CometActors
  *
  * @param dflt
  *   \- the default value to be returned if none was set prior to requesting a
  *   value to be returned from the container
  */
abstract class SessionVar[T](dflt: => T)
  ...
  /** Stateless session enforcement is new to Lift, but there are some legacy
    * issues in WebKit and allowing for a SessionVar to be "magic" (settable
    * even in stateless sessions) seems to be an efficient, yet somewhat hacky,
    * way around the issue
    */
  private[olon] def magicSessionVar_? = false
...
/** A typesafe container for data with a lifetime nominally equivalent to the
  * lifetime of HttpSession attributes. This alternative to SessionVar keeps
  * data in the container's session and must be serializable to support session
  * migration. Use SessionVars unless you are using MigratoryLiftSessions.
  *
  * <code> object MySnippetCompanion { object mySessionVar extends
  * ContainerVar[String]("hello") } </code>
  *
  * The standard pattern is to create a singleton object extending ContainerVar
  * instead of creating an instance variable of a concrete ContainerVar
  * subclass. This is preferred because ContainerVar will use the name of its
  * instantiating class for part of its state maintenance mechanism.
  *
  * If you find it necessary to create a ContainerVar subclass of which there
  * may be more than one instance, it is necessary to override the __nameSalt()
  * method to return a unique salt value for each instance to prevent name
  * collisions.
  *
  * @param dflt
  *   \- the default value to be returned if none was set prior to requesting a
  *   value to be returned from the container
  * @param containerSerializer
  *   -- an implicit parameter that keeps us honest about only storing things
  *   that can be actually serialized. Lift provides a subset of these.
  */
abstract class ContainerVar[T](dflt: => T)(implicit
  ...
  /** Different Vars require different mechanisms for synchronization. This
    * method implements the Var specific synchronization mechanism.
    *
    * In the case of ContainerVar, we don't need to do any explicit
    * synchronization. Values are stored in the HttpSession, which already gives
    * us atomic get and set operations.
    */
...
/** A trait that provides *actual* serialization of a type so that the type can
  * be stored into a container's session and be migrated across servers
  */
...
/** Create case objects that implement this trait and use the case objects to
  * denote specific SnapshotGroups for RequestVars
  */
trait RequestVarSnapshotGroup

/** This subclass of RequestVars that allow the specification of a
  * RequestVarSnapshotGroup. You can create a snapshot of all the members of
  * this group in RequestVar.snapshot
  */
...
object RequestVar {
  ...
  /** Given a RequestVarSnapshotGroup, generate a function that will snapshot
    * all the RequestVars in that group. When the function is run, the
    * RequestVars will be set to the value they held when they were snapshotted
    */
...
/** A typesafe container for data with a lifetime nominally equivalent to the
  * lifetime of a page rendered by an HTTP request. RequestVars maintain their
  * value throughout the duration of the current HTTP request and any callbacks
  * for servicing AJAX calls associated with the rendered page. RequestVar
  * instances have no value at the beginning of request servicing (excluding
  * AJAX callbacks) and their value is discarded at the end of request
  * processing. They are commonly used to share values across many snippets.
  * Basic usage:
  *
  * <code> object MySnippetCompanion { object myRequestVar extends
  * RequestVar[String]("hello") } </code>
  *
  * The standard pattern is to create a singleton object extending RequestVar
  * instead of creating an instance variable of a concrete RequestVar subclass.
  * This is preferred because RequestVar will use the name of its instantiating
  * class for part of its state maintenance mechanism.
  *
  * If you find it necessary to create a RequestVar subclass of which there may
  * be more than one instance, it is necessary to override the __nameSalt()
  * method to return a unique salt value for each instance to prevent name
  * collisions.
  *
...
abstract class RequestVar[T](dflt: => T)
  ...
  /** Is this RequestVar a member of a snapshot group? If so, specify the group
    * here
    */
  def snapshotGroup: Box[RequestVarSnapshotGroup] = Empty

  /** Return a function that, when applied, will set the value of the RequestVar
  ...
  override protected def setFunc(name: String, value: T): Unit =
    RequestVarHandler.set(name, this, value)
  ...
  override protected def registerCleanupFunc(
      in: Box[LiftSession] => Unit
  ): Unit = {
    RequestVarHandler.addCleanupFunc(in)
  }

  /** This defines whether or not Lift will log when a RequestVar is set but
    * then not read within the same request cycle. Change this to false to turn
    * off logging. Logging can also be turned off globally via
...
/** A typesafe container for data with a lifetime strictly equal to the
  * processing of a single HTTP request. Unlike ordinary RequestVar instances,
  * TransientRequestVars will not maintain data for servicing of AJAX callbacks
  * from a rendered page. This is useful in cases where the value stored within
  * the RequestVar cannot safely be used across multiple requests; an example of
  * such a value is a JTA UserTransaction which has a lifecycle strictly coupled
...
abstract class TransientRequestVar[T](dflt: => T)
  ...
  override protected def findFunc(name: String): Box[T] =
    TransientRequestVarHandler.get(name)

  override protected def setFunc(name: String, value: T): Unit =
    TransientRequestVarHandler.set(name, this, value)
          ...
          cleanup.doWith(new ListBuffer) {
            sessionThing.doWith(session) {
              ...
              if (Props.devMode && LiftRules.logUnreadRequestVars) {
                vals.value.keys.asScala
                  .filter(!_.startsWith(VarConstants.varPrefix + "olon"))
                  .filter(!_.endsWith(VarConstants.initedSuffix))
                  .foreach(key =>
                    vals.value.asScala(key) match {
...
/** Memoize a value for the duration of the current request (and subsequent Ajax
  * requests made as a result of viewing the page)
  */
