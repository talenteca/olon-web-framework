...
private[util] trait Props extends Logger {
  ...
  private[this] def interpolate(value: String): String = {
    ...
    val interpolated = for {
      case interpolateRegex(
        before,
        key,
        after
      ) <- interpolateRegex // SCALA3 compiler asked us to add "case" here
        .findAllMatchIn(
          value.toString
        )
    } yield {
  ...
  /** Determine whether the specified properties exist.
    ...
    * @return
    *   the subset of strings in 'what' that do not correspond to keys for
  ...
  /** Updates Props to find property values in the argument BEFORE looking in
    ...
    *
    * @param provider
    *   Arbitrary map of property key -> property value to be used for property
    *   lookup.
    */
  ...
  /** Passes the current `PropProvider`s to the passed `updater`, then sets the
    ...
    *
    * @param updater
    *   Function that gets the current `PropProvider`s and returns the new ones
    *   to use.
    */
  def updateProviders(
  ...
  /** Updates Props to find values in the argument when interpolating values
    * found in providers.
    *
    * @note
    *   You can only modify these BEFORE you look up any props!
    *
    * @param provider
    *   Arbitrary map of property key -> property value to be used for
    *   interpolation.
    */
  ...
  /** Passes the current `InterpolationValues`s to the passed `updater`, then
    * sets the providers to the result of the updater. Consider using
    ...
    *
    * @param updater
    *   Function that gets the current `InterpolationValues`s and returns the
    *   new ones to use.
    */
  ...
  /** The mode for which to retrieve properties, retrieved by
    * System.getProperty("run.mode"). Recognized modes are "development",
    * "test", "profile", "pilot", "staging" and "production" with the default
    * run mode being development.
    */
  ...
  /** Exposes a property affecting run-mode determination, for customisation. If
    * the property is modified after the run-mode is realised, then it will have
    * no effect and will instead log a warning indicating thus.
    *
    * @param name
    *   The property name (used to make logging messages clearer, no functional
    *   impact).
    */
  class RunModeProperty[T](name: String, initialValue: T) extends Logger {
    ...
    /** Attempts to set the property to a new value.
      *
      * @return
      *   Whether the new property was installed. `false` means modification is
      *   no longer allowed.
      */
  ...
  /** The default run-mode auto-detection routine uses this function to infer
    * whether Lift is being run in a test.
    *
  ...
  /** When the `run.mode` environment variable isn't set or recognised, this
    * function is invoked to determine the appropriate mode to use.
    *
  ...
  /** Is the app running in the Google App engine (the System property in.gae.j
    * is set)
    */
  ...
  /** The list of paths to search for property file resources. Properties files
    * may be found at either the classpath root or in /props
    */
  ...
  /** This is a function that returns the first places to look for a props file.
    * The function returns a List of String -> () => Box[InputStream]. So, if
    * you want to consult System.getProperties to look for a properties file or
...
/** Configuration management utilities.
  ...
  * in a manner relating to the context in which they are being used. The
  ...
  * "test", "staging", "production", "pilot", "profile", or "default". The
  * standard Lift properties file extension is "props".
  */
