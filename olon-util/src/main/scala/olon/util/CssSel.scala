...
/** This trait is both a NodeSeq => NodeSeq and has the ability to chain CssSel
  * instances so that they can be applied en masse to incoming NodeSeq and do
  * the transformation.
  */
...
/** This CssBind will clear all nodes marked with the class clearable. Designers
  * can mark extra nodes in markup with class="clearable" and this Bind will
  * make them go away
  */
...
private class SelectorMap(binds: List[CssBind])
    extends Function1[NodeSeq, NodeSeq] {
  ...
  private val (
    ...
    binds.foreach {
      ...
      case i @ CssBind(StarSelector(_, _)) =>
        starFunc = Full(sortBinds(i :: starFunc.openOr(Nil)))
  ...
  private abstract class SlurpedAttrs(
    ...
    final def applyRule(
      ...
      bindList match {
        ...
        // select context
        case bind :: xs if onlySelThis && isSelThis(bind) =>
        ...
        .foldLeft(elem) {
          ...
          case (_, (_, AppendKidsSubNode())) =>
            throw new RuntimeException(
              "Invalid append kids sub node apply attributes rule"
            )

          case (_, (_, DontMergeAttributes)) =>
            throw new RuntimeException(
              "Invalid dont merge attributes apply attributes rule"
            )

          case (_, (_, DontMergeClass)) =>
            throw new RuntimeException(
              "Invalid dont merge class apply attributes rule"
            )

          case (_, (_, KidsSubNode())) =>
            throw new RuntimeException(
              "Invalid kids sub node apply attributes rule"
            )

          case (_, (_, PrependKidsSubNode())) =>
            throw new RuntimeException(
              "Invalid prepend kids sub node apply attributes rule"
            )

          case (_, (_, SelectThisNode(_))) =>
            throw new RuntimeException(
              "Invalid select this node apply attributes rule"
            )

          case (_, (_, SurroundKids())) =>
            throw new RuntimeException(
              "Invalid surround kids apply attributes rule"
            )
...
trait CssBindImplicits {
  class CssBindPromoter(
    ...
    /** Transform a DOM (NodeSeq) based on rules
      ...
      * @param converter
      *   the implicit parameter that transforms T into a NodeSeq=>NodeSeq that
      *   will update things matched by the selector
      * @tparam T
      *   the type of it
      * @return
      *   the function that will transform an incoming DOM based on the
      *   transform rules
      */
    ...
    /** Transform a DOM (NodeSeq) based on rules
      ...
      *   the thing to use in the replacement rules
      * @param computer
      *   the implicit parameter that transforms T into something that will make
      *   the correct changes
      * @tparam T
      *   the type of it
      * @return
...
/** An abstract implementation of CssBind. You can instantiate this class and
  * create a custom calculate method
  */
...
final class CssJBridge extends CssBindImplicits {
  ...
  def sel(selector: String, value: NodeSeq => NodeSeq): CssSel =
    selector #> value
  ...
  /** Inserts a String constant according to the CssSelector rules
    */
  def sel(selector: String, str: StringPromotable): CssSel =
...
@implicitNotFound(
  "The value you're attempting to use on the right hand side of the #> operator can't be automatically converted to a (NodeSeq)=>NodeSeq function. Consider binding the individual members of the class you're trying to use, or define an implicit CanBind[T] that tells Lift how to convert this to a (NodeSeq)=>NodeSeq."
)
trait CanBind[-T] {
...
object CanBind extends CssBindImplicits {
  ...
  implicit def numberTransform[T <: java.lang.Number]: CanBind[T] =
  ...
  implicit def doubleTRansform: CanBind[Double] = new CanBind[Double] {
    def apply(str: => Double)(ns: NodeSeq): Seq[NodeSeq] = List(
      Text(str.toString)
    )
  }
  ...
  implicit def stringPromoteTransform: CanBind[StringPromotable] =
    new CanBind[StringPromotable] {
      def apply(str: => StringPromotable)(ns: NodeSeq): Seq[NodeSeq] = List(
        Text(str.toString)
      )
    }
  ...
  implicit def longTransform: CanBind[Long] = new CanBind[Long] {
    def apply(str: => Long)(ns: NodeSeq): Seq[NodeSeq] = List(
      Text(str.toString)
    )
  }

  implicit def boolTransform: CanBind[Boolean] = new CanBind[Boolean] {
    def apply(str: => Boolean)(ns: NodeSeq): Seq[NodeSeq] = List(
      Text(str.toString)
    )
  }
  ...
  implicit def nodeFuncTransform: CanBind[NodeSeq => Node] =
    new CanBind[NodeSeq => Node] {
      def apply(func: => NodeSeq => Node)(ns: NodeSeq): Seq[NodeSeq] = List(
        func(ns)
      )
    }
  ...
  implicit def iterableDouble[T[Double]](implicit
    ...
    new CanBind[T[Double]] {
      def apply(info: => T[Double])(ns: NodeSeq): Seq[NodeSeq] =
        f(info).toSeq.flatMap(a => List(Text(a.toString)))
    }
  ...
  implicit def stringFuncTransform: CanBind[NodeSeq => String] =
    new CanBind[NodeSeq => String] {
      def apply(info: => NodeSeq => String)(ns: NodeSeq): Seq[NodeSeq] = List(
        Text(info(ns))
      )
    }
  ...
  implicit def iterableConstFuncTransform: CanBind[IterableConst] =
    new CanBind[IterableConst] {
      def apply(info: => IterableConst)(ns: NodeSeq): Seq[NodeSeq] =
        info.constList(ns)
    }
}
