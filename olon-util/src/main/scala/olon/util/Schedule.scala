...
/** The Schedule object schedules an actor to be ping-ed with a given message
  * after a specified delay. If you need recurrent scheduled pings you will need
  * to reschedule.
  *
  * The schedule methods return a ScheduledFuture object which can be cancelled
  * if necessary
  */
...
sealed trait Schedule extends Loggable {
  ...
  /** If it's Full, then create a ArrayBlockingQueue otherwith create a
  ...
  /** Schedules the sending of a message to occur after the specified delay.
    *
    * @return
    *   a <code>ScheduledFuture</code> which sends the <code>msg</code> to the
    *   <code>to<code> Actor after the specified TimeSpan <code>delay</code>.
    */
  def schedule[T](
      to: SimpleActor[T],
      msg: T,
  ...
  /** Schedules the application of a function
    *
    * @return
    *   a <code>ScheduledFuture</code> which executes the function f immediately
    *   on a worker thread
    */
  ...
  /** Schedules the application of a function
    *
    * @return
    *   a <code>ScheduledFuture</code> which executes the function f after the
    *   delay
    */
