...
/** Generics on the JVM have an issues with Type Erasure. Basically, Generic
  * types (e.g., Function1[String, Int] and Function1[Double, Bool]) look like
  * the same type to the JVM so that methods cannot be overloaded with generic
  * types. This "marker" trait is used as a work-around to the issue. The marker
  * is implicitly passed as a parameter to some overloaded methods. If you see
  * this as an implicit parameter to an overloaded method, just ignore it.
  */
trait AvoidTypeErasureIssues1

/** The companion object that does the implicit vending of
  * AvoidTypeErasureIssues1
  */
...
trait BasicTypesHelpers { self: StringHelpers with ControlHelpers =>
  ...
  implicit class Boolean2(b: => Boolean) {
    ...
    /** Ternary operator.
      * @return
      *   a BooleanSome containing the specified value if the decorated boolean
      *   is true, or a BooleanNone otherwise.
      */
    ...
    /** Class for return values from the Boolean2 ternary operator. This class
      * provides the "|" operator that can be used to specify a default value
      * (i.e. the RHS of the "or")
      */
  ...
  /** Compare two NodeSeq and return true if they are equal, even if attribute
    * order of Elems is different
    */
  ...
  /** Optional cons that implements the expression: <code>expr ?> value :::
    * List</code>
    * @param expr
  ...
  /** A helper class that facilitates wrapping of one PartialFunction around
    * another
    */
  final implicit class PartialFunctionWrapper[A](
    ...
    /** Allows you to put a guard around a partial function such that the
      * around's isDefinedAt method must return true before the other's
      * isDefinedAt method is tested
      */
  ...
  /** A helper that will convert the String to a Boolean if it's t, true, yes,
    * 1, f, false, no, or 0
    */
