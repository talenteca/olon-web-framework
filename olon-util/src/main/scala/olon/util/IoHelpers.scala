...
trait IoHelpers {
  ...
  /** Execute the specified OS command and return the output of that command in
    * a Full Box if the command succeeds, or a Failure if an error occurs.
    */
  ...
  /** Read all data to the end of the specified Reader and concatenate the
    * resulting data into a string.
    */
  ...
  /** Read an entire file into an Array[Byte]
    */
  def readWholeFile(path: Path): Array[Byte] = readWholeStream(
...
/** A trait that defines an Automatic Resource Manager. The ARM allocates a
  * resource (connection to a DB, etc.) when the `exec` method is invoked and
  * releases the resource before the exec method terminates
  *
  * @tparam ResourceType
