...
class LAFuture[T](
  ...
  /** Satify the future... perform the calculation the results in setting a
  ...
  /** Execute the function with the value. If the value has not been satisfied,
    * execute the function when the value is satified
    */
  ...
  /** Get the future value or if the value is not satisfied after the timeout
    * period, return an Empty
    */
  ...
  def onSuccess(f: T => Unit): Unit = {
    ...
    synchronized {
      if (satisfied) {
        LAFuture.executeWithObservers(scheduler, () => contextFn(item))
      } else if (!aborted) {
        toDo ::= contextFn
      }
    }
  ...
  /** Execute a function on failure
    *
    * @param f
    *   the function to execute. Will receive a Box[Nothing] which may be a
    *   Failure if there's exception data
    */
  def onFail(f: Box[Nothing] => Unit): Unit = {
    val contextFn = LAFuture.inContext(f, context)
    synchronized {
      if (aborted)
        LAFuture.executeWithObservers(scheduler, () => contextFn(failure))
      else if (!satisfied) {
        onFailure ::= contextFn
      }
    }
  ...
  def onComplete(f: Box[T] => Unit): Unit = {
    ...
    synchronized {
      if (satisfied) {
        LAFuture.executeWithObservers(scheduler, () => contextFn(Full(item)))
      } else if (aborted) {
        LAFuture.executeWithObservers(scheduler, () => contextFn(failure))
      } else
...
object LAFuture {
  ...
  /** Create an LAFuture from a function that will be applied on a separate
    * thread. The LAFuture is returned immediately and the value may be obtained
    * by calling `get`
    *
    * @param f
  ...
  private def executeWithObservers(
  ...
  /** Do something when a future is created on this thread. This can be used to
    * see if there's any Future activity on a thread and if there is, we can do
    * smart things on an observing thread.
    *
    * @param observation
  ...
  /** Given handlers for a value's success and failure and a set of futures,
    * runs the futures simultaneously and invokes either success or failure
    * callbacks as each future completes. When all futures are complete, if the
    * handlers have not either satisfied or failed the overall result,
    * `onAllFuturesCompleted` is called to complete it. If it *still* isn't
    * complete, the overall result is failed with an error.
    *
  ...
  def collect[T, A](
    ...
    if (futures.isEmpty) {
      ...
      futures.toList.zipWithIndex.foreach { case (future, index) =>
  ...
  /** Collect all the future values into the aggregate future The returned
    * future will be satisfied when all the collected futures are satisfied
    */
  ...
  /** Collect all the future values into the aggregate future The returned
    * future will be satisfied when all the collected futures are satisfied or
    * if any of the futures is Empty, then immediately satisfy the returned
    * future with an Empty
    */
  ...
  /** Allows to wrap function in another function providing some additional
    * functionality. It may choose to execute or not execute that functionality,
    * but should not interpret or change the returned value; instead, it should
    * perform orthogonal actions that need to occur around the given
    * functionality. Typical example is setting up DB transaction.
    *
    * This is similar to `olon.common.CommonLoanWrapper`, however, it decorates
    * the function eagerly. This way, you can access current thread's state
    * which is essential to do things like set up a HTTP session wrapper
    */
  trait Context {
    def around[T](fn: () => T): () => T
