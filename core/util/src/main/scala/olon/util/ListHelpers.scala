...
/** The ListHelpers trait provides useful functions which can be applied to
  * Lists.<p/>
  */
trait ListHelpers {
  ...
  /** Compute the deltas between two sequences of a given type. Apply the
    ...
    * List of commands will be returned.
    */
  ...
  /** Compute the deltas between two sequences of a given type. Apply the
    * function based on the differences between the two lists. The resulting
    * List of commands will be returned. The algorithm used to calculate the
    * diffs is not very efficient and can degrade to O(n^2), so it's not great
    * for large collections. Internally the Seq[T] are converted to a List[T].
    * Finally, it's highly recommended that T be immutable and does proper
  ...
  /** Returns a Full Box with the first element x of the list in for which f(x)
    * evaluates to true. If f(x) evaluates to false for every x, then an Empty
    * Box is returned.
    *
    * @param in
  ...
  /** Returns the first application of f to an element of in that results in a
    * Full Box. If f applied to an element of in results in an Empty Box, then f
    * will be applied to the rest of the elements of in until a Full Box
    * results. If the list runs out then an Empty Box is returned.
    *
    * @param in
    *   a list of elements to which f can be applied
    * @param f
    ...
    *
    * @return
    *   a Box containing the first Full Box or Empty if f never returns a Full
    *   Box
    */
  def first[B, C](in: Seq[B])(_f: B => Box[C]): Box[C] = {
    val f: B => Iterable[C] = _f andThen Box.box2Iterable[C]
    // We use LazyList here to avoid multiple execution of "f" for each element access (Issue #596)
    Box(in.to(LazyList).flatMap(f).headOption)
  }

  /** This class add a case insensitive get to a List of Pairs of String, as if
    * it was a Map
    */
  class ListMapish(val theList: Seq[(String, String)]) {

    /** Return a Box containing the second element of the first pair having key
      * as the first element The comparison is made ignoring the case of the
      * keys
      *
      * @param key
      *   the string to find
      *
      * @return
      *   a Full Box containing the found value or Empty
      */
  ...
  /** Convert a java.util.Enumeration to a List[T]
    */
  def enumToList[T](_enum: java.util.Enumeration[T]): List[T] = {
    import scala.jdk.CollectionConverters._
    _enum.asScala.toList
  }

  /** Convert a java.util.Enumeration to a List[String] using the toString
    * method on each element
    */
  def enumToStringList[C](_enum: java.util.Enumeration[C]): List[String] =
    enumToList(_enum).map(_.toString)
  ...
  /** Given an incoming list, return a set of lists that is the original list
    * rotated through all its positions
    *
    * @param in
    *   the list to rotate
    *
  ...
  /** Given a list, return all the permutations including the removal of items
    * (does not return a Nil list unless in is Nil).
    *
    * @param in
    *   the list to permute
    *
    * @return
    *   all the permutations of the list including sublists, sorted in longest
    *   to shortest
    */
  ...
  implicit class SuperList[T](what: List[T]) extends AnyRef {
    ...
    /** return a new list where the element at position pos is replaced with
      * another element
      */
