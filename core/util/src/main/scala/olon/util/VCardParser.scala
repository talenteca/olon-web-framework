...
/** Parser for a VCard string such as
  *
  * BEGIN:VCARD VERSION:2.1 N:Gump;Forrest FN:Forrest Gump ORG:Bubba Gump Shrimp
  * Co. TITLE:Shrimp Man TEL;WORK;VOICE:(111) 555-1212 TEL;HOME;VOICE:(404)
  * 555-1212 ADR;WORK:;;100 Waters Edge;Baytown;LA;30314;United States of
  * America END:VCARD
  */
object VCardParser extends Parsers {
  ...
  lazy val key = elem(
    "key",
    { c => c.isLetterOrDigit || c == '-' || c == '_' || c == '.' }
  ).+ ^^ { case list =>
    val s = list.mkString; s.replaceFirst("^item\\d+\\.", "")
  }
