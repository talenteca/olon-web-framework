...
class FatLazy[T](f: => T) {
  ...
  /** Get the value of the instance. If it's not yet been set, call f to
    * calculate it
    *
    * @return
  ...
  /** Test whether the value of this class has been set or initialized from the
    * default.
    */
  ...
  /** Reset the value of this FatLazy to the default (which will be lazily
    * determined on retrieval.)
    */
...
/** Sometimes, you want to do pattern matching against a lazy value. Why?
  * Because, there may be parts of the pattern that must be evaluated first and
  * if they evaluate successfully, you then want to test another part of the
  * pattern. Thus, the LZ pattern match.
  */
object LZ {
...
/** LZ encapsulates a lazy value.
  *
  * @param f
  *   \- a value to be evaluated lazily
  */
class LZ[T](f: => T) {
  lazy val get = f
...
/** A thread-local lazy value that provides a means to evaluate a function in a
  * lazily-evaluated scope.
  *
  * @param theFunc
  *   the lazily-evaluated expression for which to cache the result in
  *   thread-local scope.
  */
class ThreadLazy[TheType](theFunc: => TheType) extends LoanWrapper {
  ...
  /** Save the current cached lazy value, if any, evaluate the specified
    * function and then restore the previous value to the cache. The effect of
    * this function is to essentially perform a reset of this lazy value to
    * being unevaluated prior to function evaluation.
    */
  def apply[T](f: => T): T = {
    val old = value.value
    calced.set(false)
  ...
  /** Reset the lazy value so that it will be recalculated from the default
    * expression on the next retrieval.
    */
