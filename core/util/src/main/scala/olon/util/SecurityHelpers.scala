...
/** The SecurityHelpers trait provides functions to:<ul> <li> generate random
  * numbers <li> generate keys <li> encrypt/decrypt keys <li> create SHA,
...
trait SecurityHelpers {
  ...
  /** return true only 'percent' times when asked repeatedly. This function is
    * used in the Skittr example to get a random set of users
    * @param percent
  ...
  /** Compare two strings in a way that does not vary if the strings are
