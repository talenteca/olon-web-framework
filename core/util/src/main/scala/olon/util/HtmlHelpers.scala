...
/** A common function-like interface for accessing information about attributes,
  * based on the two core <code>findAttr</code> and <code>convert</code>
  * methods.
  *
  * Extenders can be fairly flexible. The value of an attribute is specified by
  * the extender as type <code>Info</code>. Possibly-missing attributes are
  * returned in type Holder, which should be parametrizable. For example, you
  * could create an AttrHelper that deals in String attribute values (<code>type
  * Info = String</code>) and returns <code>Option</code>s in cases where the
  ...
  * }}}
  *
  * The helper above takes a scala <code>Elem</code> and provides a series of
  * ways to access the values of its elements. For example:
  *
  * {{{
...
trait HtmlHelpers extends CssBindImplicits {
  ...
  /** Given a NodeSeq and a function that returns a Box[T], return the first
    * value found in which the function evaluates to Full
    */
  ...
  /** Given a NodeSeq and a function that returns an Option[T], return the first
    * value found in which the function evaluates to Some
    */
  ...
  /** Finds the first `Elem` in the NodeSeq (or any children) that has an ID
  ...
  /** Adds the specified <code>cssClass</code> to the existing class attribute
    * of an Elem or create the class attribute with that class if it does not
    * exist.
    *
    * If <code>cssClass</code> is not <code>Full</code>, returns the passed Elem
    * unchanged.
    */
  ...
  /** Adds the specified <code>cssClass</code> to the existing class attribute
    * of an Elem or creates the class attribute with that class if it does not
    * exist.
    */
  ...
  private def ensureUniqueIdHelper(
      in: Seq[NodeSeq],
      processElement: (Elem, (Node) => Node) => Elem
  ): Seq[NodeSeq] = {
  ...
  /** For a list of NodeSeq, ensure that the the id of the root Elems are
    * unique. If there's a duplicate, that Elem will be returned without an id
    */
  def ensureUniqueId(in: Seq[NodeSeq]): Seq[NodeSeq] = {
    ensureUniqueIdHelper(in, (element, _) => element)
  }

  /** For a list of NodeSeq, ensure that the the id of all Elems are unique,
    * recursively. If there's a duplicate, that Elem will be returned without an
    * id.
    */
  ...
  /** Make a MetaData instance from a key and a value. If key contains a colon,
    * this method will generate a PrefixedAttribute with the text before the
    * colon used as the prefix. Otherwise, it will produce an
    * UnprefixedAttribute.
    */
