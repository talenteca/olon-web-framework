...
trait CombParserHelpers {
  ...
  /** the type of input elements defined in the Parsers trait is
    * <code>Char</code>
    */
  type Elem = Char
  ...
  /** @return
    ...
    *
    * @note
    *   Currently, it supports the BMP (Basic Multilingual Plane) set from
    *   U+0000 to U+FFFF and not the supplementary characters having code points
    *   greater than U+FFFF.
    *
    * @see
  ...
  /** @param elements
    *   list of characters
    * @return
    *   a unit parser which will succeed if the input matches the list of
  ...
  /** @return
    ...
    */
  implicit def strToLst(in: String): List[Elem] =
    new scala.collection.immutable.StringOps(in).toArray.toList
  ...
  /** @param func
    ...
    *   permutations on the list plus all permutations on sublists for example
    * @return
    *   a parser which tries the permutations of a list of parsers, given a
...
trait SafeSeqParser extends Parsers {
  ...
  /** A parser generator for non-empty repetitions.
    *
    * <p> rep1(f, p) first uses `f' (which must succeed) and then repeatedly
    * uses `p' to parse the input until `p' fails (the result is a `List' of the
    * consecutive results of `f' and `p')</p>
    *
    * @param first
    *   a `Parser' that parses the first piece of input
    * @param p
    *   a `Parser' that is to be applied successively to the rest of the input
    *   (if any)
    * @return
    *   A parser that returns a list of results produced by first applying `f'
    *   and then repeatedly `p' to the input (it only succeeds if `f' matches).
    */
  ...
  /** A parser generator for non-empty repetitions.
    *
    * <p>rep1sep(first, p, q) starts by using `first', followed by repeatedly
    * uses of `p' interleaved with `q' to parse the input, until `p' fails.
    * `first' must succeed (the result is a `List' of the consecutive results of
    * `first' and `p')</p>
    *
    * @param first
    *   a `Parser' that is to be applied to the first element of input
    * @param p
    ...
    *   `p' to the input (and that only succeeds if `p' matches at least once).
    *   The results of `p' are collected in a list. The results of `q' are
    *   discarded.
    */
