...
trait StringHelpers {
  ...
  def unquote(str: String) = {
    if (
      str != null && str.length >= 2 && str
        .charAt(0) == '\"' && str.charAt(str.length - 1) == '\"'
    )
  ...
  /** Splits a string of the form &lt;name1=value1, name2=value2, ... &gt; and
    * unquotes the quoted values. The result is a Map[String, String]
    */
  ...
  /** Replaces the value found in a string surrounded by &lt;%= ... %&gt; by a
    * replacement according to the value found in the subst Map.<p/> Throws an
    * exception if no correspondance can be found.
    *
    * @param msg
  ...
  /** Turn a string of format "FooBar" into snake case "foo_bar"
    *
    * Note: snakify is not reversible, ie. in general the following will _not_
    * be true:
    *
  ...
  /** Turn a string of format "foo_bar" into camel case with the first letter in
    * lower case: "fooBar" This function is especially used to camelCase method
    * names.
    *
    * @param name
    *   the String to CamelCase
  ...
  /** Capitalize every "word" in the string. A word is either separated by
  ...
  /** Replaces the groups found in the msg string with a replacement according
    * to the value found in the subst Map
    * @param msg
    *   string where replacements should be done
    * @param subst
    *   map of [regular expression with groups, replacement]
  ...
  /** @return
    *   an Empty can if the node seq is empty and a full can with the NodeSeq
    *   text otherwise
    */
  implicit def nodeSeqToOptionString(in: NodeSeq): Box[String] =
    if (in.length == 0) Empty else Full(in.text)

  /** Parse a string and return the Long value of that string.<p/> The string
    * can start with '-' if it is a negative number or '+' for a positive number
    * @return
  ...
  /** Encode the string to be including in JavaScript, replacing '\' or '\\' or
    * non-ASCII characters by their unicode value
    * @return
  ...
  /** Test for null and return either the given String if not null or the blank
    * String.
    */
