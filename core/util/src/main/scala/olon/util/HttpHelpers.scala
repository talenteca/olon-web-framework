...
trait HttpHelpers {
  ...
  /** The list of known suffixes used to split the URI into path parts and
    * suffixes.
    */
  val knownSuffixes: Set[String] = Set(
    "json",
    "rss",
    "atom",
    "do",
  ...
  /** Given a map of HTTP properties, return true if the "Content-type" value in
    * the map is either "text/html" or "application/xhtml+xml"
    * @param in
    *   Map which may contain a key named Content-Type
    * @return
    *   true if there is a pair ("Content-Type", "text/html") or
  ...
  def noHtmlTag(in: NodeSeq): Boolean =
    findElems(in)(_.label == "html").length != 1
  ...
  /** Transform a pair (name: String, value: Any) to an unprefixed XML attribute
    * name="value"
    */
  ...
  /** If the specified Elem has an attribute named 'id', return it, otherwise
    * construct a new Elem with a randomly generated id attribute and return the
    * pair
    *
    * @param in
  ...
  /** Given a URL and a Lift function String, append the function even if the
    * URL has query params and a #
    */
  ...
  /** Split a String at the Hash sign, run the function on the non-# side and
    * then append the hash side
    */
  ...
  /** Given a list of query parameters, append them to the URL taking into
    * account # and if there are any other query parameters
    */
  ...
  /** Get a monotonically increasing number that's guaranteed to be unique for
    * the current session
    */
  def nextNum = serial.incrementAndGet

  /** Find the elements of the specified NodeSeq that match the specified
    * predicate and concatenate them into a resulting NodeSeq.
    *
    * @param nodes
  ...
  /** Map the specified function over the elements of the specified NodeSeq and
    * return the concatenated result. This is essentially a
    * container-type-transforming flatMap operation.
    */
  ...
  /** Get a guaranteed unique field name (16 or 17 letters and numbers, starting
    * with a letter)
    */
