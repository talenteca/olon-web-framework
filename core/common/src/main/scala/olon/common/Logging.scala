...
object Logger {
  ...
  /** This function, if set, will be called before any loggers are created.
    *
    * Useful for initializing the logging backend with a non-default
    * configuration.
    *
...
trait Logger {
  ...
  /** Log the value of v with trace and return v. Useful for tracing values in
    * expressions
    */
  ...
  /** Debug a `Failure`. If the log level is debug and the `Box` is a `Failure`,
