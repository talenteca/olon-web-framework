...
class ExtractionExamples extends Specification {
  ...
  "Extract with a default value" in {
    ...
    ) mustEqual Address("Tie", "Helsinki")
  }
  ...
  "Timestamp extraction example" in {
    ...
    new Date(
      (json \ "timestamp").extract[java.sql.Timestamp].getTime
    ) mustEqual date("2009-09-04T18:06:22Z")
  }
  ...
  "Unflatten example with top level string and double" in {
    ...
    Extraction.unflatten(m) mustEqual JObject(
      List(JField("name", JString("joe")), JField("age", JDouble(32.2)))
    )
  }
...
case class OChild(
    name: Option[String],
    age: Int,
    mother: Option[Parent],
    father: Option[Parent]
)
