...
/** Basic support for heterogeneous lists, aka
  ...
  * `List` has to be a common ancestor class or trait of `Type1` and `Type2`,
  * and no such type has a `value` method.
  */
object HLists {
  ...
  /** Provides the methods that can be used on an `HList`. These are set apart
    * here due to certain issues we can experience otherwise with the type
    * variance on the `:+:` class.
    */
  implicit final class HListMethods[ListSoFar <: HList](hlist: ListSoFar)
    ...
    /** The length of this HList; note that this is O(n) in the list of
      * elements.
      */
    def length: Int = {
      hlist match {
