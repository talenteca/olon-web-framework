...
import scala.reflect.ClassTag // SCALA 3 scala.reflect.Manifest unsupported in scala 3
...
/** The Box companion object provides methods to create a Box from:
  ...
  *   - any `[[scala.AnyRef AnyRef]]` object, converting `null` to `[[Empty]]`
  *     and anything else to a `[[Full]]` with the given object
  *
...
object Box extends BoxTrait with Tryo {
  ...
  /** Helper class to provide an easy way for converting a `List[Box[T]]` into a
    * `Box[List[T]]`.
    */
  implicit class ListOfBoxes[T](val theListOfBoxes: List[Box[T]])
    ...
    /** Convert a `List` of `Box`es into a single `Box` containting a `List[T]`,
      ...
      * @return
      *   A `Full[List[T]]` if no `Failure`s were present.
      *   `ParamFailure[List[Box[T]]]` otherwise.
      */
...
private[common] sealed trait OptionImplicits {
  ...
  /** This implicit transformation allows one to use a `Box` as an `Option`.
    ...
    *
    * @return
    *   `Some` with the contents if the box is `Full` and `[[scala.None None]]`
    *   otherwise.
    */
...
sealed trait BoxTrait extends OptionImplicits {
  ...
  /** Create a `Box` from the specified `Box`, checking for `null`.
    *
    * @return
    *   `Full(in)` if `in` is a `Full` box and its value is non-null, `Empty`
    *   otherwise.
    */
  ...
  /** Transform a `List` with zero or more elements to a `Box`, losing all but
    ...
    *
    * @return
    *   `Full(x)` with the head of the list if it contains at least one element
    *   and `Empty` otherwise.
    */
  ...
  /** Apply the specified `PartialFunction` to the specified `value` and return
    ...
    *
    * @return
    *   A `Full` containing the transformed value if `pf.isDefinedAt(value)` and
    *   `Empty` otherwise.
    */
  def apply[InType, OutType](
      pf: PartialFunction[InType, OutType]
  ...
  /** This implicit transformation allows one to use a `Box` as an `Iterable` of
    ...
    *
    * @return
    *   A single-element `List` with the contents if the box is `Full` and
    *   `[[scala.collection.immutable.Nil Nil]]` otherwise.
    */
  ...
  /** Create a `Full` box containing the specified value if `in` is an instance
    * of the specified class `clz` and `Empty` otherwise.
    *
    ...
    * res0: olon.common.Box[Int] = Empty
    *
  ...
  /** Create a `Full` box containing the specified value if `in` is of type `B`
    * and `Empty` otherwise.
    *
    * For example:
    * {{{
  ...
  def asA[B](in: AnyRef)(implicit m: ClassTag[B]): Box[B] = { // SCALA3
    (Box !! in).asA[B]
  }

  // SCALA3 Experimental alternative reference to the existencial type migration
  /*
  def asA[B](in: Bar)(implicit m: ClassTag[B], conv: in.T => Bar): Box[B] = {
    (Box !! in).asA[B]
  }
...
/** The `Box` class is a container which is able to declare if it is `Full`
  * (containing a single non-null value) or `[[EmptyBox]]`. An `EmptyBox`, or
  * empty, can be the `[[Empty]]` singleton, `[[Failure]]` or
...
sealed abstract class Box[+A] extends Product with Serializable {
  ...
  /** The only time when you should be using this method is if the value is
    * guaranteed to be available based on a guard outside of the method. In
    * these cases, please provide that information in the justification
    * `String`. For example, `User.currentUser.openOrThrowException("This
    * snippet is only used on pages where the user is logged in")`. For tests,
    * use `[[==]]` or `[[===]]` instead. See the class documentation for more
    * information.
    *
    ...
    *
    * @param justification
    *   Justify why calling this method is okay and why it will not result in an
    *   exception being thrown. This serves both as mandatory documentation and
    *   as a very clear indication of what unexpected thing happened in the
    *   event you were wrong about the guard.
    *
    * @return
    *   The contents of the `Box` if it is `Full`.
    * @throws java.lang.NullPointerException
    *   If you attempt to call it on an `EmptyBox`, with a message that includes
  ...
  /** Transform an `Empty` to a `Failure` with the specified message. Otherwise
    ...
    *
    * @note
    *   This means a `Failure` will also remain unchanged; see `?~!` to change
    *   these.
    *
    * @return
  ...
  /** Transform an `Empty` or `Failure` to a `ParamFailure` with the specified
    ...
    * @return
    *   A `ParamFailure` with the specified value, unless this is already a
  ...
  /** Chain the given `msg` as a `Failure` ahead of any failures this `Box` may
    * represent.
    *
    * If this is an `Empty`, this method behaves like `[[?~]]`. If it is a
    * `Failure`, however, this method returns a new `Failure` with the given
    * `msg` and with its `[[Failure.chain chain]]` set to this `Failure`.
    *
    * As with `[[?~]]`, if this is a `Full`, we return it unchanged.
    *
    * @return
    *   A `Failure` with the message if this `Box` is an `Empty` box. Chain this
    *   box to the new `Failure` if this is a `Failure`. The unchanged box if it
    *   is a `Full`.
    */
  ...
  /** If this `Box` contains a value and it satisfies the specified `predicate`,
    ...
    *
    * @return
    *   A `Failure` with the message if the box is empty or the predicate is not
    *   satisfied by the value contained in this Box.
    */
  ...
  /** For `Full` and `Empty`, this has the expected behavior. Equality in terms
    ...
    * `Box` instead of the box itself. As a result, it is not symmetric. As an
    * example:
    * {{{
    * val foo = "foo"
    ...
    * }}}
    *
    * It is safest to use `===` explicitly when you're looking for this
    * behavior, and use `==` only for box-to-box comparisons:
    * {{{
  ...
  /** Transforms this box using the `transformFn`. If `transformFn` is defined
    * for this box, returns the result of applying `transformFn` to it.
    * Otherwise, returns this box unchanged.
    *
    * If you want to change the content of a `Full` box, using `[[map]]` or
    * `[[collect]]` might be better suited to that purpose. If you want to
    * convert an `Empty`, `Failure` or a `ParamFailure` into a `Full` box, you
    * should use `[[flip]]`.
    *
    * @example
    *   {{{
    *
    * // Returns Full("alternative") because the partial function covers the
    * case. Full("error") transform { case Full("error") => Full("alternative")
    * }
    *
    * // Returns Full(1), this Full box unchanged, because the partial function
    * doesn't cover the case. Full(1) transform { case Full(2) =>
    * Failure("error") }
    *
    * // Returns this Failure("another-error") unchanged because the partial
    * function doesn't cover the case. Failure("another-error") transform { case
    * Failure("error", Empty, Empty) => Full("alternative") }
    *
    * // Returns Full("alternative") for an Empty box since `partialFn` is
    * defined for Empty Empty transform { case Empty => Full("alternative") }
    *
    * // Returns Empty because the partial function is not defined for Empty
    * Empty transform { case Failure("error", Empty, Empty) =>
    * Full("alternative") }
    *
    *   }}}
    */
  def transform[B >: A](
      transformFn: PartialFunction[Box[A], Box[B]]
  ): Box[B] = {
    transformFn.applyOrElse(this, (thisBox: Box[A]) => thisBox)
  }

  /** Returns a `Full` box containing the results of applying `flipFn` to this
    * box if it is a `Failure`, `ParamFailure` or `Empty`. Returns `Empty` if
    * this box is `Full`. In other words, it "flips" the full/empty status of
    * this Box.
    */
...
sealed abstract class EmptyBox extends Box[Nothing] with Serializable {
  ...
  def asA[B](implicit m: scala.reflect.ClassTag[B]): olon.common.Box[B] = this
...
sealed case class Failure(
  ...
  /** Return a list of the exceptions that led to this `Failure`. First,
    * unflattens the list of causes of this `Failure`'s `exception`. Then, if
    * this `Failure` has a `chain`, walks down it and concatenates their
    * `exceptionChain` to the end of this one's.
    *
    * @return
    *   A single list of `Throwable`s from the most direct cause to the least
    *   direct cause of this `Failure`.
    */
  ...
  /** Reduce this `Failure`'s message and the messages of all chained failures a
    ...
    * res0: olon.common.Failure = Failure(It's all sideways,Empty,
