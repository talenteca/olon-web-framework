...
/** Implements an LRU Hashmap. Given a size, this map will evict the least
  ...
  *
  * @param initmaxSize
  *   The initial max size. This can be updated using `[[updateMaxSize]]`.
  * @param loadFactor
  *   If non-`Empty`, specifies the load factor for the backing
  *   `java.util.HashMap`.
  * @param expiredFunc
  *   When a key-value pair is removed, the last thing that happens is that
  *   these functions are invoked. Note that this happens after `[[expired]]` is
  *   invoked.
  */
class LRUMap[K, V](
    initMaxSize: Int,
  ...
  /** Unsafe version of `[[get]]`.
    ...
    *   If the key does not exist in the map. Use `get` instead to get a safe
    *   `[[Box]]` result that can be checked for existence, or use
    *   `[[contains]]` before calling this.
    */
  ...
  /** Override this method to implement a test to see if a particular element
    * can be expired from the cache.
    */
