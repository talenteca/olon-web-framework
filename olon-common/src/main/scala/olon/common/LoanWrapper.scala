...
/** A component that takes action around some other functionality. It may choose
  ...
  * change the returned value; instead, it should perform orthogonal actions
  * that need to occur around the given functionality. A canonical example is
  * wrapping an SQL transaction around some piece of code.
  *
  * As an example, this trait defines the principal contract for function
  * objects that wrap the processing of HTTP requests in Lift.
  */
...
object CommonLoanWrapper {
  ...
  /** If you have a `List` of `LoanWrapper`s, apply them and then the functions.
    * For example:
    *
    * {{{
