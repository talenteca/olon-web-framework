...
trait BaseGetPoster {
  ...
  def post[RT](
      ...
      capture: (String, HttpClient, HttpMethodBase) => ResponseType,
      bodyToRequestEntity: RT => RequestEntity
  ): ResponseType = {
    val poster = new PostMethod(baseUrl + url)
    poster.getParams().setCookiePolicy(CookiePolicy.RFC_2965)
    for ((name, value) <- headers) poster.setRequestHeader(name, value)
...
trait TestKit extends ClientBuilder with GetPoster with GetPosterHelper {
  ...
  class TestHandler(res: TestResponse) {
    def _then(f: TestResponse => TestResponse): TestResponse = f(res)
...
object TestHelpers {
  ...
  /** Given an HTML page, find the list of "lift_toWatch" names and values These
    * can be fed back into a comet request
    *
    * @param body
  ...
  /** Given the body of a Comet response, parse for updates "lift_toWatch"
    * values and update the current sequence to reflect any updated values
    *
    * @param old
...
/** A response returned from an HTTP request. Responses can be chained and
  * tested in a for comprehension: <pre>
  *
  * implicit val
  *
  * for { login &lt;- post("/api/login", "user" -> "me", "pwd" -> "me") !@
  * "Failed to log in" info &lt;- login.get("/api/info") if info.bodyAsString
  * must_== "My Info" moreInfo &lt;- info.post("/api/moreInfo", &lt;info&gt;Some
  * Info&lt;/info&gt;) } moreInfoheaders("X-MyInfo") must_== List("hello",
  * "goodbye") </pre>
  */
trait Response {
  type SelfType
  ...
  /** Test the response as a 200. If the response is not a 200, call the
  ...
  /** Test that the server gave a response. If the server failed to respond,
  ...
  /** Test that the server gave a particular response code. If the response is
    * not a 200, call the errorFunc with the msg
    *
    * @param msg
    *   the String to report as an error
    * @param errorFunc
  ...
  /** Test that the server response contains a particular node anywhere in the
    * xml with the exact attributes and children. If the response does not
    * contain valid xml or does not contain the exact node, call the errorFunc
    * with the msg.
    *
    * @param node
  ...
  def \\(node: Node, msg: => String)(implicit
      errorFunc: ReportFailure
  ): SelfType

  /** Test that the server response contains a node with a particular label
    * anywhere in the xml. If the response does not contain valid xml or does
    * not contain the node, call the errorFunc with the msg
    *
    * @param label
  ...
  /** Test that the server response does not contain a particular node anywhere
    * in the xml with the exact attributes and children. If the response does
    * contain valid xml and does contain the exact node, call the errorFunc with
    * the msg.
    *
    * @param node
    *   the XML node to search for in the response
  ...
  /** Test that the server response does not contain a node with a particular
    * label anywhere in the xml. If the response does contain valid xml and does
    * contain the node, call the errorFunc with the msg
    *
    * @param label
    *   the label for the XML node to search for in the response
    * @param msg
  ...
  /** Test that the server response contains a particular node as a direct child
    * with the exact attributes and children. If the response does not contain
    * valid xml or does not contain the exact node, call the errorFunc with the
    * msg.
    *
    * @param node
    *   the XML node to search for in the response
  ...
  /** Test that the server response contains a node with a particular label as a
    * direct child. If the response does not contain valid xml or does not
    * contain the node, call the errorFunc with the msg
    *
    * @param label
    *   the label for the XML node to search for in the response
  ...
  /** Test that the server response does not contain a particular node as a
    * direct child with the exact attributes and children. If the response does
    * contain valid xml and does contain the exact node, call the errorFunc with
    * the msg.
    *
    * @param node
  ...
  /** Test that the server response does not contain a node with a particular
    * label as a direct child. If the response does contain valid xml and does
    * contain the node, call the errorFunc with the msg
    *
    * @param label
    *   the label for the XML node to search for in the response
  ...
  /** the Response has a filter method for chaining in a for comprehension. Note
    * that the filter method does *NOT* have to return a Boolean, any expression
    * (e.g., an assertion)
    */
  def filter(f: FuncType => Unit): FuncType
}

/** The response to an HTTP request, as long as the server responds with
  * *SOMETHING*
  */
class HttpResponse(
    baseUrl: String,
