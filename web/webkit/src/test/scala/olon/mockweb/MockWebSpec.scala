...
/** System under specification for MockWeb. This does the double duty as both a
  * spec against the MockWeb object as well as an example of how to use it.
  */
class MockWebSpec extends Specification {
  ...
  /** We can create our own LiftRules instance for the purpose of this spec. In
    * the examples below we can call
    * LiftRulesMocker.devTestLiftRulesInstance.doWith(mockLiftRules) {...}
    * whenever we want to evaluate LiftRules. For simpler usage, WebSpecSpec
    * provides full-featured LiftRules mocking.
    */
