...
trait JxBase {
  ...
  def addAttrs(varName: String, attrs: List[MetaData]): JsCmd = attrs
    .map { m =>
      m.value
        .map {
          case exp: JsExp =>
  ...
  def addToDocFrag(parent: String, elems: List[Node]): JsCmd = elems
    .map {
      ...
      case a: Atom[_] =>
        JsRaw(
          parent + ".appendChild(document.createTextNode(" + a.text.encJs + "));"
        ).cmd
