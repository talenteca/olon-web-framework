...
/** A container for accumulating `[[JsCmd]]`s that need to be sent to the
  * client. When `[[toResponse]]` is called to finalize the response, in
  * addition to the JS passed directly to this instance, the commands in
  * `[[S.jsToAppend]]` are also read and included in the response. Also in this
  * process, all of the `JsCmd` instances have their `toJsCmd` methods called to
...
case class JsonCall(funcId: String) {
  ...
  def apply(command: String, params: JsExp) =
    JsCmds.Run(
      funcId + "({'command': " + command.encJs + ", 'params':" +
        params.toJsCmd + "});"
    )

  def apply(command: String, target: String, params: JsExp) =
    JsCmds.Run(
      funcId + "({'command': " + command.encJs + ", 'target': " +
        target.encJs +
        ", 'params':" +
        params.toJsCmd + "});"
    )
  ...
  def apply(command: JsExp, params: JsExp) =
    JsCmds.Run(
      funcId + "({'command': " + command.toJsCmd + ", 'params':" +
        params.toJsCmd + "});"
    )

  def apply(command: JsExp, target: JsExp, params: JsExp) =
    JsCmds.Run(
      funcId + "({'command': " + command.toJsCmd + ", 'target': " +
        target.toJsCmd +
        ", 'params':" +
        params.toJsCmd + "});"
    )
...
/** The companion object to JsExp that has some helpful conversions to/from
  * Lift's JSON library
  */
object JsExp {
  import json._
...
object JE {
          ...
          JsRaw(
            name + ".appendChild(lift$.swappable(" + AnonFunc(
              ...
              + "(), " + AnonFunc(
                JsCmds.JsCrVar(
                  "df",
                  JsRaw("document.createDocumentFragment()")
                ) &
  ...
  object LjBuildIndex {
    ...
    def apply(obj: JsExp, indexName: String, tables: (String, String)*): JsExp =
      new JsExp {
        def toJsCmd =
  ...
  protected trait MostLjFuncs {
    ...
    def apply(obj: JsExp, func: JsExp): JsExp = new JsExp {
      def toJsCmd =
        "lift$." + funcName + "(" + obj.toJsCmd + ", " + func.toJsCmd + ")"
    }
  }
  ...
  object LjAlt {
    def apply(obj: String, func: String, alt: String): JsExp = new JsExp {
      def toJsCmd =
        "lift$.alt(" + obj + ", " + func.encJs + ", " + alt.encJs + ")"
    }
  ...
  /** A JavaScript method that takes parameters
    *
    * JsFunc is very similar to Call but only the latter will be implicitly
    * converted to a JsCmd.
    * @see
    *   Call
    */
  ...
  /** A JavaScript method that takes parameters
    *
    * Call is very similar to JsFunc but only the former will be implicitly
    * converted to a JsCmd.
    * @see
    *   JsFunc
    */
...
trait HtmlFixer {
  ...
  /** Calls fixHtmlAndJs and if there's embedded script tags, construct a
    * function that executes the contents of the scripts then evaluations to
    * Expression. For use when converting a JsExp that contains HTML.
    *
    * @note
  ...
  def fixHtmlFunc(uid: String, content: NodeSeq)(f: String => String) =
    fixHtmlAndJs(
      uid,
      content,
      forceExtractInlineJavaScript = Some(false)
    ) match {
      case (str, Nil) => f(str)
      case (str, cmds) =>
  ...
  /** Calls fixHtmlAndJs and if there's embedded script tags, append the JsCmds
    * to the String returned from applying the function to the enclosed HTML.
    * For use when converting a JsCmd that contains HTML.
    */
  ...
  /** Super important... call fixHtml at instance creation time and only once
    * This method must be run in the context of the thing creating the XHTML to
    * capture the bound functions
    */
...
""")
  ...
  /** Replaces the content of the node with the provided id with the markup
    * given by content
    *
    * This is analogous to assigning a new value to a DOM object's innerHtml
    * property in Javascript.
    *
    * @param id
    *   \- the id of the node whose content will be replaced
    * @param content
    *   \- the new content
    */
  ...
  /** Creates a JavaScript function with a name, a parameters list and a
    * function body
    */
  object Function {
...
"""
  ...
  /** Sets the value to the element having the 'id' attribute with the result of
  ...
  /** Assigns the value computed by the 'right' expression to the 'left'
    * expression.
    */
  ...
  /** Creates a JavaScript var named by 'name' and assigns it the value of
    * 'right' expression.
    */
  ...
  /** Assigns the value of 'right' to the members of the element having this
    * 'id', chained by 'then' sequences
    */
  ...
  case class After(time: TimeSpan, toDo: JsCmd) extends JsCmd {
    def toJsCmd =
      "setTimeout(function() {" + toDo.toJsCmd + "}, " + time.millis + ");"
  }
  ...
  object RedirectTo {
    ...
    /** Redirect to a page and execute the function when the page is loaded
      * (only if the page is on the same server, not going to some other server
      * on the internet)
      */
  ...
  case class JsDoWhile(body: JsExp, condition: JsExp) extends JsCmd {
    def toJsCmd =
      "do { " + body.toJsCmd + " } while ( " + condition.toJsCmd + " )"
  }
