...
/** Base class for things that require pagination. Implements a contract for
  * supplying the correct number of browsable pages etc
  *
  * @tparam T
  *   the type of item being paginated
  * @author
  *   nafg and Timothy Perrett
  */
trait Paginator[T] extends Loggable {
  ...
  /** Returns a list of page numbers to be displayed in 'zoomed' mode, i.e., as
    * the page numbers get further from the current page, they are more sparse.
    */
  def zoomedPages = (
...
/** In many situations you'll want to sort things in your paginated view.
  * <code>SortedPaginator</code> is a specialized paginator for doing such
  * tasks.
  *
  * T: The type of the elements, accessed via def page within the listing
  * snippet C: The type of the columns, used to specify sorting
  *
  * @author
  *   nafg and Timothy Perrett
  */
trait SortedPaginator[T, C] extends Paginator[T] {
  ...
  /** The sort headers: pairs of column labels, and column identifier objects of
    * type C.
    */
  ...
  /** Returns a new SortState based on a column index. If the paginator is
    * already sorted by that column, it toggles the direction; otherwise the
    * direction is ascending. Note that this method does not alter the sort
    * state in the paginator; it only calculates the direction toggle. Example
    * usage: sortedPaginator.sort =
    * sortedPaginator.sortedBy(columns.indexOf(clickedColumn))
    */
...
/** This is the paginator snippet. It provides page navigation and column
  * sorting links.
  *
  * The values for the pagination are bound according to the classes specified
  * in the [[paginate]] method, using a CSS selector transform.
  *
  * @author
  *   nafg and Timothy Perrett
  */
trait PaginatorSnippet[T] extends Paginator[T] {
  ...
  /** Overrides the super's implementation so the first record can be overridden
  ...
  /** Returns XML that links to a page starting at the given record offset, if
    * the offset is valid and not the current one.
    * @param ns
    *   The link text, if the offset is valid and not the current offset; or, if
    *   that is not the case, the static unlinked text to display
    */
...
/** This trait adds snippet functionality for sorted paginators. You can place
  * bind points in your template for column headers, and it turns them into
  * links That you can click to sort by that column. Simply write, e.g.,
  * &lt;th&gt;&lt;sort:name/&gt;&lt;/th&gt;&lt;th&gt;&lt;sort:email/&gt;&lt;/th&gt;
  * etc.
  */
trait SortedPaginatorSnippet[T, C]
  ...
  /** This method binds template HTML based according to the specified
    ...
    *
    * For example, with a list of headers `List("foo", "bar")`, this would bind
    * the `.foo` element's contents to contain a link to a page that renders
    * that column sorted, as well as the `.bar` element's contents to contain a
    * link to a page that renders that column sorted.
    */
...
/** Sort your paginated views by using lifts functions mapping. The only down
  * side with this style is that your links are session specific and
  * non-bookmarkable. If you mix this trait in to a StatefulSnippet, it should
  * work out the box. Otherwise, implement 'registerThisSnippet.'
  * @author
  *   nafg and Timothy Perrett
  */
trait StatefulSortedPaginatorSnippet[T, C]
  ...
  /** This method is called before the new page is served, to set up the state
    * in advance. It is implemented by StatefulSnippet so you can just mix in
    * StatefulSortedPaginatorSnippet to one; or you can implement it yourself,
    * using things like S.mapSnippet.
    */
