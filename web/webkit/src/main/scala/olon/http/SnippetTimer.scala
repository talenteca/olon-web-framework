...
/** A snippet timer is a general interface for timing snippets. A few default
  * implementations are provided and can be selected by setting
  * LiftRules.snippetTimer as you need.
  */
trait SnippetTimer {
...
object NoOpSnippetTimer extends SnippetTimer {
  override def timeSnippet(snippetName: String)(
      snippetFunc: => NodeSeq
  ): NodeSeq = snippetFunc
}
...
object LoggingSnippetTimer extends SnippetTimer {
  override def timeSnippet(
      snippetName: String
  )(snippetFunc: => NodeSeq): NodeSeq = {
