...
import olon.actor.{LiftActor, LAFuture}
...
class LiftRules() extends Factory with FormVendor with LazyLoggable {
  ...
  /**
   * A session identifier for [[olon.http.provider.servlet.HTTPServletSession]].
   *
  ...
  */
  ...
  configureLogging = olon.util.LoggingAutoConfigurer()
...
private[http] case object DefaultBootstrap extends Bootable {
  def boot(): Unit = {
    val f = createInvoker("boot", Class.forName("bootstrap.olon.Boot").newInstance.asInstanceOf[AnyRef])
    f.map {f => f()}
  }
}
