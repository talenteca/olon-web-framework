...
/** The trait that forms the basis for LiftScreen and the Screen instances in
  * Wizard
  */
trait AbstractScreen extends Factory with Loggable {
  ...
  /** Add a FieldContainer to the Screen. A FieldContainer can contain either a
    * single field (a BaseField) or a collection of BaseFields. The key
    * take-away is that if the LiftScreen or Wizard is a singleton, you can
    * still display variable number of fields by returning a variable number of
    * BaseField instances from the FieldContainer. <br/> WARNING -- this method
    * is public so it can be called from a Wizard. This method should only be
    * called from within the Screen or Wizard that owns the Screen and not from
    * external code.
    */
  ...
  protected def hasUploadField: Boolean =
    screenFields.foldLeft(false)(_ | _.uploadField_?)
  ...
  protected case object NothingOtherValueInitializer
      extends OtherValueInitializer[Nothing]

  protected case class OtherValueInitializerImpl[T](f: () => T)
      extends OtherValueInitializer[T]
  ...
  trait Field extends ConfirmField {
    ...
    protected def otherValueDefault: OtherValueType =
      null.asInstanceOf[OtherValueType]

    /** A field my have an "otherValue" which can be used to store a list of
      * options or other information that the fields needs on a Screen/Wizard by
      * Screen/Wizard basis
      */
    ...
    def set(v: ValueType) =
      _currentValue.set(setFilter.foldLeft(v)((nv, f) => f(nv)))
  ...
  protected class FieldBuilder[T](
    ...
    /** Add a filter field (the wacky symbols are supposed to look like a filter
      * symbol)
      */
    ...
    /** Add a validation (the wacky symbols are supposed to look like a check
      * mark)
      */
  ...
  /** Create a FieldBuilder so you can add help screens, validations and
    * filters. Remember to invoke "make" on the returned FieldBuilder to convert
    * it into a field
    *
    * @param name
    *   \- the name of the field. This is a call-by-name parameter, so you can
    *   dynamically calculate the name of the field (e.g., localize its name)
    * @param default
  ...
  protected object FilterOrValidate {
    ...
    implicit def promoteValidate[T](
        v: T => List[FieldError]
    ): FilterOrValidate[T] = AVal(v)

    implicit def promoteToFormParam(
        a: SHtml.ElemAttr
    ): FilterOrValidate[Nothing] = FormParam(a)
    ...
    ): FilterOrValidate[Nothing] = AFieldBinding(binding)
  }

  sealed protected trait FilterOrValidate[+T]

  /** Override the screen default for fields appearing on the confirm screen and
    * force this field to appear on the confirm screen
    */
  protected case object OnConfirmScreen extends FilterOrValidate[Nothing]

  /** Override the screen default for fields appearing on the confirm screen and
    * force this field not to appear on the confirm screen
    */
  protected case object NotOnConfirmScreen extends FilterOrValidate[Nothing]

  protected case class FormParam(fp: SHtml.ElemAttr)
  ...
  protected case class AVal[T](v: T => List[FieldError])
      extends FilterOrValidate[T]

  protected case class AFieldBinding(binding: FieldBinding)
      extends FilterOrValidate[Nothing]

  protected case class Help(ns: NodeSeq) extends FilterOrValidate[Nothing]

  protected case class FieldTransform(func: BaseField => NodeSeq => NodeSeq)
      extends FilterOrValidate[Nothing]
  ...
  /** A little hack because => BaseField and => Box[BaseField] have the same
    * method signature
    */
  ...
  /** Create a field with a name, default value, and
    ...
    * @param name
    *   \- the name of the field. This is a call-by-name parameter, so you can
    *   dynamically calculate the name of the fiels (e.g., localize its name)
    * @param default
    *   \- the default value of the field
    * @param validate
  ...
  /** A validation helper. Make sure the string is at least a particular length
    * and generate a validation issue if not.
    */
  ...
  /** Create a field that's added to the Screen
    *
    * @param theName
    *   the name of the field. This is call-by-name, so you can do things like
    *   S.?("Dog's Name") such that the string will be localized
    *
    * @param defaultValue
    *   the starting value for the field. This is also call-by-name which is
    *   handy for constructs like: <code
    *   class="scala">SomeExternalRequestVarOrSessionVar.get</code>
    *
    * @param theToForm
    *   a function to convert the field into a form
    *
    * @param otherValue
    *   a handy way include other values in the field. The other value is
    *   calcualted when the field is initialized. You can, for example, put a
    *   list of valid options in the field.
    *
    * @param stuff
  ...
  /** Create a select HTML element
    ...
    *   \- a list of filters and validations for the field
    * @param f
    *   a PairStringPromoter (a wrapper around a function) that converts T =>
    *   display String
    *
    * @return
    *   a newly minted Field{type ValueType = String}
...
trait ScreenWizardRendered extends Loggable {
  ...
  private object FieldBindingUtils {
    def sel(f: CssClassBinding => String, sel: String) =
      sel format (f(cssClassBinding))
    def replace(f: CssClassBinding => String) =
      ".%s" format (f(cssClassBinding))
    def replaceChildren(f: CssClassBinding => String) =
      ".%s *" format (f(cssClassBinding))
  ...
  protected def renderAll(
    ...
    val notices: List[(NoticeType.Value, NodeSeq, Box[String])] =
      S.getAllNotices
  ...
  /** Should all instances of this Wizard or Screen default to Ajax when not
    * explicitly set
    */
  ...
  /** Are the forms Ajax or regular HTTP/HTML.
    *
    * If the ajax=true attribute is present on the original snippet invocation,
    * the forms will be ajax.
    */
...
trait LiftScreen
  ...
  protected object LocalActions
      extends ScreenVar[AtomicReference[Map[String, () => JsCmd]]](
  ...
  /** Holds the template passed via the snippet for the duration of the request
    */
  protected object _defaultXml
      extends TransientRequestVar[NodeSeq](NodeSeq.Empty) {
    override lazy val __nameSalt = Helpers.nextFuncName
  }
  ...
  protected object PrevSnapshot
      extends TransientRequestVar[Box[ScreenSnapshot]](Empty) {
    override lazy val __nameSalt = Helpers.nextFuncName
  }
  ...
  /** A unique GUID for the form... this allows us to do an Ajax SetHtml to
    * replace the form
    */
  ...
  /** What to do when the Screen is done. By default, will do a redirect back to
    * Whence, but you can change this behavior, for example, put up some other
    * Ajax thing or alternatively, remove the form from the screen.
    */
  ...
  /** Keep request-local information around without the nastiness of naming
    * session variables or the type-unsafety of casting the results. RequestVars
    * share their value through the scope of the current HTTP request. They have
    * no value at the beginning of request servicing and their value is
    * discarded at the end of request processing. They are helpful to share
    * values across many snippets.
    *
    * @param dflt
  ...
  abstract class ScreenVar[T](dflt: => T) extends NonCleanAnyVar[T](dflt) {
    override protected def findFunc(name: String): Box[T] =
      ScreenVarHandler.get(name)

    override protected def setFunc(name: String, value: T): Unit =
      ScreenVarHandler.set(name, this, value)
  ...
  protected def renderHtml(): NodeSeq = {
    ...
    def fieldTransform(
        field: BaseField
    ): List[BaseField => NodeSeq => NodeSeq] =
      field match {
  ...
  protected def doFinish(): JsCmd = {
    ...
    if (!LocalAction.get.isEmpty)
      fMap.get(
        LocalAction.get
      ) map (_()) getOrElse (throw new IllegalArgumentException(
...
object FieldBinding {
  ...
  /** Bind the field using the default template defined in an external template
    * (as in Bind.helpers()-based binding)
    */
  case object Template extends BindingStyle

  /** Bind the field using the template defined in the body of the field
    * reference
    */
  case object Self extends BindingStyle

  /** Bind the field using the template returned by the
    * <code>defaultFieldNodeSeq</code> method
    */
  ...
  /** Bind the field using the results of a function. The provided function will
    * be called every time the field is rendered.
    */
