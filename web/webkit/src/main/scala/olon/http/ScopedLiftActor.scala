...
/** A LiftActor that runs in the scope of the current Session, repleat with
  * SessionVars, etc. In general, you'll want to use a ScopedLiftActor when you
  * do stuff with clientActorFor, etc. so that you have the session scope
  */
trait ScopedLiftActor extends LiftActor with LazyLoggable {

  /** The session captured when the instance is created. It should be correct if
    * the instance is created in the scope of a request
    */
  ...
  /** The session associated with this actor. By default it's captured at the
    * time of instantiation, but that doesn't always work, so you might have to
