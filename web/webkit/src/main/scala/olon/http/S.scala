...
trait S extends HasParams with Loggable with UserAgentCalculator {
  ...
  // callback.
  private[olon] object requestCometVersions extends TransientRequestVar[Set[CometVersionPair]](Set.empty)
  ...
  **/
   ...
   *
   * @see olon.http.provider.HTTPCookie
   ...
   *
   * @see Req.uri
   * @see olon.http.provider.HTTPRequest.uri
   */
   ...
   * method that can be used as a log function for the olon.mapper.DB.addLogFunc
   * method to enable logging of Mapper queries. You would set it up in your bootstrap like:
   *
   * <pre name="code" class="scala" >
   * import olon.mapper.DB
   * import olon.http.S
   * class Boot  {
   *   def boot  {
   *     ...
   ...
   *
   * <pre name="code" class="scala" >
   * import olon.http.{ ResourceVar,S }
   * import olon.util.LoanWrapper
   *
   ...
   * can use the helpful Extractor in olon.util.JsonCommand
   *
