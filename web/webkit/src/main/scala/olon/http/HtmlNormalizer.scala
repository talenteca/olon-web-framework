...
/** Represents an HTML attribute for an event handler. Carries the event name
  * and the JS that should run when that event is triggered as a String.
  */
private case class EventAttribute(eventName: String, jsString: String)
private object EventAttribute {

  /** Some elements allow a URL attribute to take a `javascript:(//)`-style URL
    * instead of setting an `on*` event in order to invoke JS. For example, you
    * can (and Lift does) set a form's `action` attribute to `javascript://(some
    * JS)` instead of setting `onsubmit` to `(someJS); return false`.
    *
    ...
    if (attributes == Null) {
      ...
      attributes match {
        ...
        case UnprefixedAttribute(name, attributeValue, _)
            if name.startsWith("on") && extractInlineJavaScript =>
          val updatedEventAttributes =
