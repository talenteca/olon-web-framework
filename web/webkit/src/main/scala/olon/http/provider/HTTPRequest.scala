...
trait HTTPRequest {
  ...
  /** @return
    *   \- HTTP authentication scheme: BASIC, DIGEST etc. Empty if there is auth
    *   header.
    */
  ...
  /** @return
    *   \- all header parameters present in this request. Nil if no headers are
    *   used.
    */
  def headers: List[HTTPParam]

  /** @return
    *   \- the context path. Similar with HttpServletRequest.getContextPath.
    *   Return "" empty string if your implementation does not support the
    *   contept of context path
    */
  def contextPath: String

  /** @return
    *   \- the HTTPContext of this service. It does not guarantee that it
    *   returns the same HTTPContext reference upon each invocation but it must
    *   guarantee that HTTPContext reference contains the same information.
    */
  ...
  /** @return
    *   \- Some[Any] if this is a resumed request, return the state associated
    *   with it.
    */
  ...
  /** Resume this request
    * @return
    *   false if this continuation cannot be resumed as it is not in pending
    *   state.
    */
  ...
  /** @return
    *   \- the locale forthis request. Empty if there is not language
    *   information.
    */
  ...
  /** Creates a new HTTPRequest instance as a copy of this one. It is used when
    * snapshots of the current request context is created in order for this
    * request object to be used on different threads (such as asynchronous
    * template fragments processing). The new instance must not keep any
    * reference to the container' instances.
    */
