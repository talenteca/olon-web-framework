...
trait Loc[T] {
  ...
  /** Override this if the default request value should be something other than
    * Empty
    */
  ...
  /** A `PartialFunction` that maps a snippet name, and an optional `Loc` value,
  ...
  /** Is the Loc marked as Stateless (this will force rendering of the page into
    * stateless mode)
    */
  ...
  /** A lazy val used to track statelessness for non-dev mode. By default, it
    * calls calcStateless().
    */
  protected lazy val _frozenStateless = calcStateless()

  /** The method to calculate if this Loc is stateless. By default looks for the
    * Loc.Stateless Param
    */
  ...
  protected def _testAccess: Either[Boolean, Box[() => LiftResponse]] = {
    def testParams(
        what: List[Loc.LocParam[T]]
    ): Either[Boolean, Box[() => LiftResponse]] = what match {
      case Nil => Left(true)
  ...
  /** The template assocaited with this Loc, if any. Any Loc.Template or
  ...
  /** The title to be displayed for the value associated with this Loc. Any
  ...
  def buildKidMenuItems(kids: Seq[Menu]): List[MenuItem] = {
    kids.toList.flatMap(
      _.loc.buildItem(Nil, false, false)
...
trait ConvertableLoc[T] {
  ...
  /** Converts the String to T that can then be sent to the Loc in createLink
    */
...
object Loc {
  ...
  /** Create a Loc (Location) instance
    *
    * @param name
    *   -- the name of the location. This must be unique across your entire
    *   sitemap. It's used to look up a menu item in order to create a link to
    *   the menu on a page.
    * @param link
  ...
  def apply(
      ...
      link: Link[Unit],
      text: LinkText[Unit],
      params: LocParam[Unit]*
  ): Loc[Unit] = UnitLoc(name, link, text, params.toList)
  def apply(
      name: String,
      link: Link[Unit],
      text: LinkText[Unit],
      params: List[LocParam[Unit]]
  ): Loc[Unit] = UnitLoc(name, link, text, params)
  ...
  /** Algebraic data type for parameters that modify handling of a Loc in a
    * SiteMap
    */
  trait LocParam[-T] {
  ...
  /** Indicates that the path denominated by Loc requires HTTP authentication
    * and only a user assigned to this role or to a role that is child-of this
    * role can access it.
    */
  ...
  /** Tests to see if the request actually matches the requirements for access
    * to the page. For example, if a parameter is missing from the request, this
    * is a good way to restrict access to the page.
    */
  case class Test(test: Req => Boolean) extends AnyLocParam

  /** If the test returns True, the page can be accessed, otherwise, the result
    * of FailMsg will be sent as a response to the browser. If the Loc cannot be
    * accessed, it will not be displayed in menus.
    *
    * @param test
    *   -- the function that tests access to the page
    * @param failMsg
    *   -- what to return the the browser (e.g., 304, etc.) if the page is
    *   accessed.
    */
  ...
  /** MenuCssClass is used to add css to the Menu node. The css allows for
    ...
    * cssClass is a StringFunc which can either be a String constant or a
    * Function that returns a String. Thus, you can compute the css based on the
    * current state or you can have a constant. Syntactically you can use
    * either: <pre> MenuCssClass("foobar") MenuCssClass(() =>
    * calculateCssForMyMenuItem()) </pre>
    */
  case class MenuCssClass(cssClass: StringFunc) extends AnyLocParam

  /** Unless the test returns True, the page can be accessed, otherwise, the
    * result of FailMsg will be sent as a response to the browser. If the Loc
    * cannot be accessed, it will not be displayed in menus.
    *
    * @param test
    *   -- the function that tests access to the page
    * @param failMsg
  ...
  /** Allows extra access testing for a given menu location such that you can
    * generically return a response during access control testing
    */
  ...
  /** Allows a user to specify a template based upon a function from the current
    * value encapsulated in the Loc. Allow the return of Box[NodeSeq] so that
    * it's more friendly to Templates.
    */
  case class TemplateBox(template: () => Box[NodeSeq]) extends AnyLocParam
  case class ValueTemplateBox[T](template: Box[T] => Box[NodeSeq])
      extends LocParam[T]

  /** This LocParam may be used to specify a function that calculates a title
    * for the page based upon the current value encapsulated by this Loc.
    */
  case class Title[T](title: T => NodeSeq) extends LocParam[T]

  /** If the Loc is in a group (or groups) like "legal" "community" etc. the
    * groups can be specified and recalled at the top level
    */
  case class LocGroup(group: String*) extends AnyLocParam

  /** Calculate the value for the Loc. This is useful for parameterized menus.
    * It allows you to calculate the value of the Loc.
    */
  ...
  /** The common interface for `LocParam`s that provide snippet functions, which
    * can be aware of the `Loc` value.
    * @tparam A
  ...
  trait ValueSnippets[A] extends LocParam[A] {
    ...
    /** Provides snippets for the `Loc`.
      * @return
      *   a `PartialFunction` that maps a snippet name, and an optional `Loc`
      *   value, in a `Tuple2`, to a snippet function (`NodeSeq` => `NodeSeq`).
      */
    def snippets: PartialFunction[(String, Box[A]), NodeSeq => NodeSeq]
  }
  object ValueSnippets {

    /** Factory for general `ValueSnippets` instances by wrapping an existing
      * `PartialFunction`.
      * @tparam A
      ...
      */
    def apply[A](
        pf: PartialFunction[(String, Box[A]), NodeSeq => NodeSeq]
    ): ValueSnippets[A] = new ValueSnippets[A] {
      def snippets = pf
    }
  }

  /** A single snippet that's associated with a `Loc`, but is not directly aware
    * of the `Loc` value.
    * @see
    *   ValueSnippets
    * @param name
    *   the snippet name
    * @param _func
    *   the snippet function. Note that this is a call-by-name parameter; that
    *   is, the function expression passed in will be evaluated each time the
    *   function is invoked.
    */
  ...
  object Snippet {
    ...
    /** Build a Loc.Snippet instance out of a name and a DispatchSnippet (or
      * StatefulSnippet, LiftScreen or Wizard). The "render" method will be
      * invoked on the Dispatch snippet
      */
    def apply(name: String, snippet: => DispatchSnippet)(implicit
        disambiguate: DummyImplicit
    ): Snippet =
      new Snippet(name, ns => snippet.dispatch("render")(ns)) // Issue #919

    /** Build a Loc.Snippet instance for a given name and a function. Note that
      * the function is call-by-name so that it will be created each time it's
      * used. This is useful for CSS Selector Transforms
      */
  ...
  /** A subclass of LocSnippets with a built in dispatch method (no need to
    * implement isDefinedAt or apply... just def dispatch:
  ...
  /** If this parameter is included, the item will not be visible in the menu,
  ...
  /** If this parameter is included, the Loc will continue to execute even if
    ...
    * if its currentValue comes up Empty, and as a result will return an HTTP
    * 404. For situations where this is not the desired, "Not Found" behavior,
    * you can add the MatchWithoutCurrentValue LocParam to a Loc, then use the
    * IfValue LocParam to define what should happen when the currentValue is
    * Empty.
    *
  ...
  /** A function that calculates the statelessness of the Loc for the given
    * request with the parameterized type passed into the function
    */
  ...
  /** The companion object to LinkText that contains some helpful implicit
    * conversion
    */
  object LinkText {
  ...
  /** This defines the Link to the Loc.
    ...
    *
    * @param matchHead_?
    *   -- false -- absolute match. true -- match anything that begins with the
    *   same path. Useful for opening a set of directories (for example, help
    *   pages)
    */
  class Link[-T](val uriList: List[String], val matchHead_? : Boolean)
    ...
    /** Returns the value created by createPath wrapped in a boxed
      * scala.xml.Text element. NOTE: This does not create a clickable HTML link
      * on its own!
      */
  ...
  object Link {
    ...
    implicit def strPairToLink(in: (Seq[String], Boolean)): Link[Unit] =
      new Link[Unit](in._1.toList, in._2)
  }
