...
object ThreadPoolRules {
  ...
  /** When threads are created in the thread factories, should they null the
    * context class loader. By default false, but it you set it to true, Tomcat
    * complains less about stuff. Must be set in the first line of Boot.scala
    */
  @volatile var nullContextClassLoader: Boolean = false
}

/** LAPinger is for scheduling LiftActors to be pinged with an arbitrary message
  ...
  def schedule[T](
    ...
    try {
      ...
      case e: RejectedExecutionException =>
        throw PingerException(s"${msg} could not be scheduled on ${to}", e)
    }
  }
